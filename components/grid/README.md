...
|---|---|---|
| `theme` | `default` / `muted` / `inverted` band tone | `_band` → `background.fill`; the `muted` framing is `border.width-top`/`width-bottom` + `style` + `color`; the `inverted` ink is `_band` → `typography.color`, which `heading` follows through `currentColor` — **but `subheading` does not**, so a dark band is at least two writes |
...
| `items[].text_role` | `mono` / `meta` / `label` / `kicker` preset on a card's text | the `card-text` role's `udc` map on that item, or a custom preset (#1016). Measured: `mono` and `meta` rendered byte-identically to the default above 767px — two of the four values had no rendered effect at all |
...
- **Darkening the `_band` does not reach `subheading` or `empty`:** both pin `@color-muted`, and `empty` sits on the band fill rather than inside a card.
