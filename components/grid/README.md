...
One slot value genuinely retired with no route, named here rather than left to be discovered: `--grid-featured-texture-color` (a second background layer the `fill` grammar refuses). `--grid-item-bullet-color` has a route since #1028: `card-bullets` -> `marker.color` (the check mark is a `::before` no role addresses, so the param sets a colour on the list that each check inherits; per card too, through an item's own `udc`). Unset, the check takes the accent.
