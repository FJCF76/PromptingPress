...
These are the keys every action returns, not the complete set for every action. An action may add its own: `restore_composition` builds its own `findings` (#233), unbounded, and the accepted-write attachment leaves it exactly as that action built it. Key on what you need rather than on the exact key set.
