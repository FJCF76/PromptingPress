...
2c. **Restore reports, never blocks (#233).** `restore_composition` replays a stored history snapshot, so it is the one mutation surface that may legally write a composition current validators reject: a snapshot captured before a rule existed still restores. Undo is wired to it (`assets/js/pp-ai-chat.js`), so a restore that current rules refuse would make undo fail exactly when a user most needs it. The rule is therefore *report, don't block*: restore preserves the snapshot verbatim (chrome included — **never strip content from history**) and returns a `findings` array describing what current rules say. **Two PRECONDITIONS can still refuse it, and neither is a validation veto. (#818):** a ring slot may hold bytes that did not decode to a composition, preserved so a corrupt-page repair cannot destroy the only copy of them. There is nothing to replay, so restore refuses with `history_entry_not_restorable` — the same species as `no_history` / `history_out_of_bounds`. **(#829):** the selector is resolved against the ring the caller read and CONFIRMED against the authoritative ring inside the write lock, so a concurrent write that moves what `steps_back` / `history_index` names is refused with `history_target_shifted` and writes nothing, rather than replaying a snapshot the operator never addressed behind an `ok:true`. That one bites `steps_back` hardest — it is RELATIVE, so every concurrent write changes what it means — and the answer is to re-list the ring and re-select, preferring a `history_index` from the current listing. Both are statements about WHICH SLOT the selector names, never judgments on its contents. Do not read that as a crack in this invariant: no validation rule vetoes any slot that carries a composition. **"A composition" means a LIST OF COMPONENTS (#841, #842) — shorthand for a structural test, spelled out in the paragraph below.** The ring asks one shared predicate at both ends — when it stores a prior and when it reads one back — because two shapes used to slip through as replayable snapshots and fatal on replay. The CONTAINER half (#841): a JSON OBJECT decodes to a PHP associative array and passed an `is_array()` test; replaying one either rewrote a healthy page back into a corrupt one or, when the object was a single band (`{"component":"hero",…}`), crashed `pp_update_composition()` outright. The ELEMENT half (#842): a payload whose container IS a list but whose entries are not bands — `["a","b"]`, or a list holding `{"component":"x","props":"str"}` — drove the id-injection loop onto a string and raised an uncaught `TypeError`, from a slot the listing had just called `restorable: true` on a page the classifier called HEALTHY. So the replayability rule is the one rule restore does NOT ignore, and it is not ignoring a validation rule when it refuses: the slot holds no composition to replay, and the bytes are handed back through `wp pp operate composition-history` instead. Everything INSIDE a replayable snapshot is still replayed verbatim and reported, which is the invariant this paragraph is about.

**Keep the replayability test and the validity test apart — that boundary IS the #233 contract.** The predicate (`_pp_history_payload_is_snapshot()`, `lib/wp.php`) asks only what replay structurally requires: the container is a list, every entry is an array, and any `props` present is an array. It does NOT ask what a validator asks. A band with no `component` key is INVALID and still restores, reporting the problem through the shared engines; `[["a","b"]]` is nonsense to every renderer and still restores. If you ever find yourself adding a schema question to that predicate, you are building the second validator this repo forbids, and you are breaking #233. The line to hold is STRUCTURAL: a container is required and a scalar sits there. That is a slightly wider set than the shapes that literally raise (`false`/`null` entries and a `props` of `false` are rewritten rather than raised on, and are refused with their family), and a much narrower one than "is this composition legal". Stated precisely, because the boundary is narrower than "object": `{"0":…,"1":…}` decodes to a genuine PHP list (the keys ARE `0..n-1`), so nothing below the decode can tell it from an authored array and it is still snapshotted and replayed — only a genuinely string- or gap-keyed object reaches the refusal. Since #604 "verbatim" is literal — no key is canonicalized on the way in, so a snapshot carrying a retired prop name or a pre-#69 `variant` restores with those names intact and is told about them in `findings`. Findings come from the two shared engines, `pp_validate_composition_errors()` and `pp_validate_composition_smells()` — never a restore-specific validator. Any new validation rule you add to those engines is inherited for free by restore's report, by the read-only diagnostics `wp pp check page` / `wp pp validate site` (same `_pp_composition_findings()` since #622), and since #687 by every ACCEPTED composition write's `findings` envelope key; that is the point. **A rule you add to `pp_validate_composition_errors()` must report and CONTINUE, never end the item (#621).** Gate every finding on `_pp_claim_item_finding($sink, <role>, ...<locator>)` — using the same locator your message names, so the claim granularity matches the message granularity — then continue to the next prop / entry / field. Two rules that can judge the same value must not both report it, and a rule that skips a value another rule already owns asks `_pp_item_finding_claimed()` first. Only the four structural checks (no `component` key, non-scalar `component`, unknown component, template-owned chrome) end an item, because nothing below them can be judged. `pp_validate_composition()` still rejects on `errors[0]`, so the write path keeps its single actionable message — since #642 it returns that error with the BAND named in the message (`Component 1 ("logos") prop ...`) and the offset carried on the envelope as `index`, which is a rendering step, not a rule; the collected findings keep the band-free wording every reporting surface prints its own locator beside; rule ORDER is therefore load-bearing (it decides which message a rejected write shows), and reordering rule blocks is a behavior change. **An action that validates something which is not the page's composition calls `pp_validate_composition_item()`, never `pp_validate_composition()` on a synthetic array (#642)** — offset 0 of a one-item wrapper is not the page's band 0, and reporting it would send an agent to repair an unrelated stored band. `add_component` is the shipped example. `pp_update_composition()` (`lib/wp.php`) stays a non-validating writer: the action layer owns validation.
...
4. **Style-slot & token values are validated at write AND at render:** the write-time engine `_pp_validate_token_value` (`lib/apply.php`) is authoritative, and `pp_render_style_vars` (`lib/wp.php`) re-validates each stored value at the render boundary through that same shared engine before emitting it inline (defense-in-depth for values that reach storage without passing write-time validation, e.g. a restored snapshot per #233 or a raw DB write). A value the boundary rejects is dropped from output only; the page and any restore still succeed. Never render a raw slot value. Both gates ("is the slot declared?" and "does the value pass?") live behind the single predicate `pp_style_declaration_renders()` (`lib/wp.php`) — call that rather than re-deriving either test. **Design-token overrides have the same shape** (#965): `pp_token_overrides_inline_css()` (`lib/wp.php`) builds the `:root` block only from rows that clear `pp_token_override_renders()`, a row that fails is dropped rather than emitted, and `pp_check_token_override_validity()` reports the drop through `pp_preflight()` using that same predicate — so the page and the advisory cannot disagree about what paints.
...
5. **Escaping at output:** `esc_html` for plain-text fields, `esc_url` for URLs, `esc_attr` for attributes, `wp_kses_post` for rich HTML fields (the main prose surfaces: `section.body`, `faq.answer`), and `pp_kses_inline` (`lib/helpers.php`, #439) for supporting-text fields that allow a bounded inline subset (`a, strong, em, br`) but no block elements (`cta.body`, `grid.items[].text`, `testimonials.items[].quote`). Each text prop's schema `description` states its contract. Do not widen `esc_url` protocols globally, and do not widen the inline allowlist per-component.
   ...
   **`esc_url` and `wp_kses_post` need a GUARDED local, exactly like `pp_render_style_vars` does (#730).** This is the one escaping rule that is not obvious from the signatures, because both are WordPress CORE functions and both are **untyped**:

   ```php
   $raw_button_url = $props['button_url'] ?? '#';
   $button_url     = is_scalar($raw_button_url) ? (string) $raw_button_url : '';
   // ... later: esc_url($button_url)
   ```

   An untyped parameter reads as safe and is not. Each reaches a string-only PHP builtin before it makes any sanitization decision, so a stored array or object raises a `TypeError` from *inside core*: `esc_url()` in `ltrim()`, `wp_kses_post()` in `str_contains()` on an array and `preg_replace()` on an object. Measured on real WordPress 7.0 / PHP 8.3. `templates/composition.php` does not catch it, so that is a **whole-page 500**, not one dropped fragment. The value arrives from data the write path never saw — a restored snapshot (#233), a pre-rule composition, a raw `_pp_composition` write.

   `is_scalar`, **not** `is_string`: PHP runs coercive here (no `declare(strict_types)` anywhere in this theme), so only non-scalars ever fataled, and `is_string` would silently drop a stored int/float/bool that renders fine. Since #707 the front door no longer *accepts* those — a `type: "string"` prop rejects a non-string scalar at write — but that narrowing gates WRITES, not storage, and the guard's whole job is the data no write gate ever saw: a pre-#707 composition, a restored snapshot (#233), a raw meta write. Those still hold `42`, and `42` still has to render. Keep `is_scalar` here; #707 closed the front door this guard was built to survive, it did not replace the guard. Read the prop exactly ONCE into `$raw_<name>`; every gate below reads the guarded local. Where a prop's contract is an ARRAY, use the `is_array($raw) ? $raw : []` form instead (#708/#739) — every scalar fatals at an `array` parameter.

   **Never `try`/`catch` a `wp_kses_post()` `TypeError` to recover.** Core's `wp_pre_kses_block_attributes()` runs `remove_filter('pre_kses', …)`, then `filter_block_content()`, then the matching `add_filter()`. A throw escaping the middle step means the re-add never runs, so swallowing it de-registers block-attribute KSES for the rest of the request — trading a visible 500 for an invisible sanitization hole. Guard before the call.

   `esc_html`/`esc_attr` do **not** need this: they coerce an array to the literal string `Array` with an `E_WARNING` rather than fataling. That is a real defect of its own (#736/#721), just not this one.

   `tests/InvariantTest.php` (`testEveryCoreEscaperCallInAComponentTakesAGuardedLocal`) fails the build on any argument reaching either sink without a matching guard assignment in the same file — it is keyed on the GUARD, not on the variable name, so reusing a name another component already guards will not satisfy it. `tests/StoredLinkAndRichTextRenderGuardTest.php` pins the degradation in emitted HTML and compares the call-site set to its declared inventory by equality.

   **The obligation follows the prop into shared helpers (#742).** A component's guard only covers the reads that component performs. A shared `lib/` helper that RE-READS a prop it was handed reads the raw stored value again, downstream of every component guard: `pp_render_faq_schema()` takes faq's `items` and reads each entry's `question` and `answer` with its own `(string)` cast and its own array-offset read. Guard inside the helper, using the same two forms. Two things make this boundary easy to miss. First, both `InvariantTest` checkers scan `components/` only, so a helper that drifts fails nothing — the pin has to be behavioural. Second, the constructs are different: a `(string)` cast and an array offset read are LANGUAGE constructs, not typed calls and not core escapers, so the failure modes differ again — an object fatals at the cast (or at the offset read, with `Cannot use object of type X as array`), while an array coerces to the literal word `Array` with only an `E_WARNING`, wherever that output happens to go. In `pp_render_faq_schema()` it went into the page's FAQPage JSON-LD, which no human reads.
...
3. **Render:** the slot is emitted by `pp_render_style_vars($style, '{name}')` in `components/{name}/{name}.php`, where `$style` is the **guarded** local — never the raw prop (#708):
   ...
   $slot_style = pp_render_style_vars($style, '{name}');
   ```
...
5. **Decide composable vs chrome (#223).** If a page places it, it is composable — give it at least one required prop, or a bare `{"component":"x"}` validates while the accordion round-trip cannot preserve it (`SchemaValidationTest::testEveryComposableComponentDeclaresARequiredProp()` pins this). If instead `templates/base.php` renders it on every page, it is chrome: add it to `pp_template_owned_components()` (`lib/admin.php`) so `pp_validate_composition()` rejects it from a composition, and to `pp_template_owned_menu_locations()` (`lib/wp.php`) if it reads a nav-menu location it renders on EVERY page. **Chrome has a styling surface now (#976):** declare a `roles` block in its schema exactly as a composable v2 component does, emit `data-pp-chrome="<name>"` on its root element, and its `udc` map lives in the `pp_site_udc` site option rather than in a composition — `UdcEngineTest` fails if a component declares roles and never emits its scoping attribute. Chrome declares role `defaults` exactly as a composable v2 component does, and its stylesheet block keeps only structure (#994 retired the empty-defaults arrangement for nav and footer together). The defaults tier emits on EVERY site, styled or not — there is no stored-entry gate — because the resting appearance IS the defaults. A location it renders only when a menu is assigned goes in `pp_conditionally_rendered_menu_locations()` instead (#582) — the two lists answer different questions, and a conditional location in the always-on list warns every site that never opted in. The drift guards in `tests/NavReadinessTest.php` read `templates/base.php` back and fail if either list disagrees with it.
