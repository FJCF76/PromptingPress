...
| Unreadable-composition **batch refusal** (#749) | Data-safety | Batch executor only (`pp_ai_execute_batch`) + the chat batch entry point (`_pp_ai_execute_batch_response`), both asking one owner (`_pp_batch_unreadable_refusal`) so they cannot drift. Single-step execute, editor AJAX and `pp_patch_composition()` are **not** covered — and deliberately so: they take no rollback snapshot, so they have no snapshot to write back over the corrupt bytes. Since #756 the refusal admits ONE batch shape, the repair it prescribes: a lone `update_composition`/`restore_composition` step on an already-corrupt page (see the row below). Single-step-only, because with a second step a later rollback writes the degraded `[]` snapshot over the repair that just landed |
| Rollback **withhold** on an unreadable capture (#749, widened #756) | Data-safety | `_pp_restore_batch_snapshot()`, the other half of the carve-out. Two branches, and the order matters: a page the snapshot recorded as unreadable never has its composition written back at all (its capture is the degrading accessor's `[]`, not a composition the page ever had), and a page that was readable at capture but is unreadable NOW is withheld by the live re-classify. The first is what stops the carve-out's own rollback erasing the repair it just admitted — a case the live check cannot catch, because a repaired page reads fine. Both report through `rollback_errors` (#755: `rolled_back: true` is not clean until you check it) |
