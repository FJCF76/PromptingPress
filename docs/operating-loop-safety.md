...
re-reading is the repair and the model has nothing to fix. What the card SAYS beside that
affordance is gated on evidence since #797: a conflict at a FAILING STEP means earlier steps
ran and were rolled back, so the card claims "Nothing was applied." only when the rollback
report is explicitly clean, and otherwise names which pages or menus stayed dirty through
the same disclosure the executed-failure card uses (#755). **Since #854 that clean claim is
also worth what it says.** The gating was honest about the CHANNEL from the day it landed
and could not be honest about the channel's COVERAGE: a batch that created a redirect or
imported an attachment reported an empty list while both survived, so the strongest sentence
in the client was gated on a report that had never looked at the two step classes most
likely to leave something behind. The rollback now removes what its own batch created and
...
| Advisory lock **survives its own connection** (#830, ruling 2026-09-01) | Data-safety | Shared engine (`_pp_with_advisory_lock`), inherited by all THREE lock families — the per-post composition lock, the install-scoped token-override lock, and the per-run operate-state lock (`pp_operate_with_run_lock`, #409) — and therefore by every reader and writer inside any of those critical sections. MySQL advisory locks are connection-scoped, and `wpdb::query()` self-heals a dropped connection: on errno 2006 it reconnects and re-runs the statement on a NEW connection, reporting success with an empty `last_error`. The reconnect voided the `GET_LOCK` for the whole rest of the mutator, so the compare-and-swap above, the ring rebuild and all three meta writes ran **unlocked** with no signal of any kind — the lost update the lock exists to prevent, arriving by a door nothing was watching. The retry budget (`$wpdb->reconnect_retries`) is now suspended for the duration of the section and restored on every exit, so **wpdb's own self-heal can no longer relocate the section** — a deliberately narrower claim than "the section cannot be relocated". Two routes stay open and nothing in the theme can shut them: third-party code reassigning `$GLOBALS['wpdb']` mid-request (nothing here does), and a connection-multiplexing proxy (ProxySQL, RDS Proxy) that can hand the client session a different backend session between statements with no errno 2006 raised, so no retry budget is ever consulted. A `wpdb` **subclass** is outside it too: HyperDB and LudicrousDB keep several live connections and route between them, and LudicrousDB replaces `check_connection()` outright, so such a handle accepts the suspension while the behaviour it gates no longer exists — and on a replica-routing config `SELECT GET_LOCK(...)` can route to the read group while the writes go to the primary, meaning the lock never covered the writes to begin with. The acquire itself is deliberately left un-suspended: a `GET_LOCK` that self-heals re-runs on the surviving connection and genuinely holds the lock there. **The guarantee is "never runs unlocked", not "always refuses"**, and the difference is worth stating because four shapes reach the caller. A dead connection terminates the request inside core (`check_connection()` → `bail()` / `dead_db()`), which is fail-closed but is core's process kill rather than an envelope, and skips the `finally` — MySQL's connection-close auto-release is the backstop there, as it already was for a hard fatal. **That is the only shape a shipped write path reaches today**: the two below require `did_action('template_redirect')`, which core tests before the bail branch, and every composition, token and run-state write originates in WP-CLI, admin-AJAX or the activation seed, none of which reach the front-end template loader. Three consequences worth knowing before you meet one: the theme emits nothing of its own (core dies inside `query()` and never returns), and what the operator sees depends on the surface — `dead_db()` loads a `wp-content/db-error.php` drop-in if one exists, then branches on `wp_installing() || defined( 'WP_ADMIN' )`, so the dashboard editor and the chat (admin-AJAX defines `WP_ADMIN`) get core's verbose **"Error reconnecting to the database"**, which does name the reconnect, while WP-CLI and any front-end caller get the terse **"Error establishing a database connection"**; under WP-CLI a multi-step run aborts with the run lock's `finally` skipped, leaving that run's state row un-finalized; and a transient 2006 that used to self-heal now ends the whole request. The trade is right — an unlocked write is worse than a failed one — but it is a real change in what an operator sees (tracked, with the traceless-termination consequence, in #943). **One shape is a genuine regression and is named rather than buried:** the death does not have to land on a read. `pp_update_composition()` writes three meta rows in order (`_pp_composition`, `_pp_composition_hash`, `_pp_composition_version`), and a connection that dies BETWEEN the first and the second leaves the new composition persisted under the OLD hash and version — a freshness marker certifying content that is not there, which the next writer's compare-and-swap then clears and clobbers. Pre-#830 that window usually self-healed, so suspending the retries widens it from "every reconnect attempt failed" to "the connection died". Three meta writes are not one atomic unit and this change does not make them one; closing it is #857-shaped work. After `template_redirect` core returns instead of bailing, the in-lock version read answers 0 through the #212 blind spot (a `check_connection()` failure returns null with `last_error` EMPTY), and a caller carrying a **non-zero** `expected_version` is refused with the existing `composition_conflict` — no new refusal class was minted for this. The third shape is the residual, and it is two caller shapes rather than one: a caller that supplies **no** baseline (`create_page`, the homepage seed, legacy direct writes), and a caller that supplies **`expected_version = 0`** on a never-written page — the dead read answers 0 as well, so the CAS compares 0 against 0 and PASSES, which is the same documented back-compat path by which an absent marker initializes to version 1. Neither has a gate that can catch it, so the meta writes fail on the dead connection and the write returns optimistically over a page nothing changed. Closing that needs `update_post_meta()`'s verdict to be readable, which is the half of the #821 ruling deferred to the #857 compare-first idiom. **To check whether the invariant is actually in force on an install**, grep the error log for `PromptingPress: could not suspend $wpdb auto-reconnect` — that line (emitted once per PHP process, per distinct cause) means the handle could not be guarded and the section is running with the full pre-#830 exposure. Its absence is the healthy state. One property worth knowing before writing a plugin against this theme: the suspension is a property of the **connection**, so it covers everything running inside the section, third-party callbacks on the meta hooks included. On a healthy connection it is unobservable — `query()` consults `check_connection()` only when the handle is empty or errno is 2006, its sole call site in the class, so a live connection never reads the budget at all — and the difference appears only on a connection that has already died mid-section, where a plugin's write was equally unlocked before |
...
| History-ring **skipped-push disclosure** (#821, ruling 2026-09-01) | Data-safety | Shared choke point (`pp_update_composition`), inherited by every caller. The ring push is guarded against an encode failure: the previous entries are KEPT rather than clobbered with a `false`, and the composition write PROCEEDS anyway — guard and disclose, not block, which is the narrow first step the ruling took. The disclosure is what makes the narrowness safe: the accepted write carries a `history_not_recorded` entry leading its `findings`, so a change with no undo point can no longer look like an ordinary one. Consumers that COUNT findings ask `pp_finding_is_about_the_composition()` so the entry is never mistaken for a validation-rule problem. **One neighbouring failure stays silent by design**: a ring push that fails inside `update_post_meta()`, which returns `false` for an unchanged value as well as a real failure, so gating on it waits on a compare-first read — deferred by the same ruling. **The other neighbour is no longer silent (#941, 1.19.11):** `pp_update_composition()`'s own composition encode was unchecked, so a composition the encoder rejected was stored as an EMPTY value while the call returned `true` and the page read back as a healthy blank one. It now REFUSES with `composition_not_encodable` before the lock is acquired, which is the opposite posture to the ring guard beside it and deliberately so — a failed ring push costs the caller its undo point, while that write IS the loss |
...
| Rollback **removal** of what the batch created (#854, ruling T1) | Data-safety | Batch executor only (`pp_ai_execute_batch` records, `_pp_restore_batch_snapshot` removes). The half a snapshot cannot express: a page, an attachment or a `pp_redirects` row that did not exist before the batch has no pre-batch state to write back, so the executor records each one as its step SUCCEEDS and the rollback removes it. Two things are load-bearing. First, recording happens on SUCCESS, not at snapshot time: the snapshotter sees the steps a batch intends to run, and only the executor sees which ran, so a row named by a step that never executed is left exactly as found. Second, the scope is drawn at CREATION rather than at writing — an `import_media` that deduped to an existing attachment (`action: "reused"`) is never recorded, and a `create_redirect` that overwrote an existing row restores the prior `to` + `code` from a per-key pre-batch baseline instead of deleting it. The redirect restore patches only the rows the batch actually wrote, over a fresh read, so a row another admin wrote during the batch window survives (the failure mode `_pp_restore_menu_state()` still has, recorded in TODOS.md). Where a delete is unsafe (an ID that no longer addresses an attachment) or a write is refused, the entry is refused and the survivor is NAMED in `rollback_errors` — which is what makes an empty report mean "nothing survived" instead of "this step class was never covered", the fail-open the #797 gating was quietly resting on. The hole #854 left open on purpose is now closed by #857: every write the restorer makes checks its own return, including the `wp_delete_post()` that removes a created page, so a failed restore names what it could not put back instead of reporting clean |
...
| Unreadable-composition **batch refusal** (#749) | Data-safety | Batch executor only (`pp_ai_execute_batch`) + the chat batch entry point (`_pp_ai_execute_batch_response`), both asking one owner (`_pp_batch_unreadable_refusal`) so they cannot drift about WHICH batches refuse, and since #833 one owner for WHAT a target is classified from too (`_pp_batch_target_refusal_reason`, used by both detectors and by the rollback's re-classify). Single-step execute, editor AJAX and `pp_patch_composition()` are **not** covered — and deliberately so: they take no rollback snapshot, so they have no snapshot to write back over the corrupt bytes. Since #756 the refusal admits ONE batch shape, the repair it prescribes: a lone `update_composition`/`restore_composition` step on an already-corrupt page (see the row below). Single-step-only, because with a second step a later rollback writes the degraded `[]` snapshot over the repair that just landed. **Since #833 the refusal classifies from the database row, and fails closed.** It used to classify through the object cache while the carve-out beside it read the row, so a request holding a stale-healthy cached copy over a corrupt row reported nothing and the batch simply ran (the rollback baseline was then captured from that stale copy). A target now blocks unless BOTH reads classify it clean — the cached check survives as an added requirement, never as an alternative source, which is what keeps the cache-corrupt/row-healthy refusal #756 pinned and what lets the snapshot keep capturing its baseline from the cached read. The gate blocks with `composition_unverifiable` whenever the row cannot be read, which is TWO cases: no usable database handle (`pp_composition_db_handle`), a unit-context and exotic-host path since production WordPress always has one; and a row query that FAILED (`$wpdb->last_error` non-empty), which is reachable in production under exactly the contention that produces the race — `get_var()` reports a killed query, a lock-wait timeout or a gone-away connection the same way it reports an absent row, and the authoritative reader maps that to a blank READABLE page, so trusting it at a gate was a fail-open. Same `#212` disambiguation the three sibling authoritative readers make. Neither case says anything about the page and neither prescribes a repair. A batch naming no existing page classifies nothing and is never refused for a database it never needed |
| Rollback **write verification** (#857) | Data-safety | `_pp_restore_batch_snapshot_report()`. The other half of what makes `rollback_errors: []` trustworthy, and the half #854 deliberately left for its own slot: coverage without verification is only half the claim. Every write the restorer performs used to be fire-and-forget, so a write that was REFUSED left the applied state in place and reported nothing — a created page still live, a site setting still holding the batch's value, the Custom CSS still deleted, all behind "all changes in this proposal have been reverted". Each write now checks its own return and names what it could not put back. Two rules keep the new entries honest, both learned from the mirror-bug this channel already has an issue about (#855). First, an AMBIGUOUS return is disambiguated before it is believed: `update_option()` and `delete_option()` return false both for a refused write and for a write with nothing to do, so those five writes compare the live state first and skip when it already matches — past that guard a false return is provably a refusal. Second, nothing that is provably GONE is reported as a survivor: a created page core answers `null` for (no row, as against a refused delete) and a page deleted inside the batch window both stay silent, because nothing survived. The stated limits are unchanged from #854 and are the concurrency cluster's axis, not this one's: each writer's return is trusted rather than verified by reading the value back, and the compare-then-write pair is not atomic. **Since #876 there is no longer a corner of the rollback's own code whose writes are unchecked.** The menu layer (`_pp_restore_menu_state()`) used to be that corner — it reported the items it could not RECREATE while discarding every `wp_delete_post()` inside `pp_clear_nav_menu_items()` and the `set_theme_mod()` that restores the location map. Both now report, on the same channel, under the same `failed` tag. Two limits are named rather than implied: `wp_delete_nav_menu()` deletes item rows and zeroes locations inside core and returns only the term deletion's result, so an orphan item row under a deleted batch-created menu stays invisible; and the location comparison reads through the `theme_mod_nav_menu_locations` filter while the write is raw, so under a filtering plugin the guard can skip a write that was owed (the same filtered-vs-raw question TODOS.md already parks for the snapshotter) |
| Rollback **withhold** on an unreadable capture (#749, widened #756) | Data-safety | `_pp_restore_batch_snapshot_report()`, the other half of the carve-out. Two branches, and the order matters: a page the snapshot recorded as unreadable never has its composition written back at all (its capture is either the degrading accessor's `[]` or, since #833, a cached copy the authoritative row contradicts — unconfirmable either way, which is why it is never written back), and a page that was readable at capture but is unreadable NOW is withheld by the live re-classify. The first is what stops the carve-out's own rollback erasing the repair it just admitted — a case the live check cannot catch, because a repaired page reads fine. Both report through `rollback_errors` (#755: `rolled_back: true` is not clean until you check it) — and since #797 all three of the chat's batch-failure exits honor that rule, so no surface narrates a clean revert the channel does not support. **Since #853 those exits are also REACHED.** Honoring the rule was a property of the exits, not of the client: `executeProposal()` walked `batch.steps` before testing which exit applied, so an envelope whose `steps` arrived as anything but a list threw there and the operator got the TypeError's text — `Error: batch.steps.forEach is not a function` — in place of the rollback report the rule exists to deliver. The shape is now checked before it is walked, so a malformed envelope reaches its exit and keeps its report (that report reads `rollback_errors`, never `steps`); what it loses is the failing step's own quote — and, since #872, the step NUMBER too when `failed_at` is not a non-negative integer, because the quote is read through that same index. Three of those malformed values (`'0'`, `0.5`, `-0.5`) previously threw out of the skipped-steps pass instead, which took the whole exit and its report with them, so the reach this row claims was not complete until #872 closed them. The one exit that refuses instead is success: a batch claiming `ok: true` that cannot evidence the claim is not narrated as an apply, because that would write a false confirmation into the model's conversation as well as the operator's card. Since #853 that meant a `steps` nobody can read; since #871 (ruling T5) it also means a READABLE list carrying fewer step results than the proposal had steps, which used to narrate the subset it did report. Both are one condition now. The live re-classify reads the same way the refusal does since #833 — the database row, failing closed with no handle — because asking the object cache there echoed the snapshot-time verdict on installs without a persistent object cache, which is the same hole one step later. **Since #855 a withhold no longer READS as a failure.** The channel carried both meanings as opaque strings, so the card drew a protection in the failure colour and told the operator a change could not be reverted over a page nothing had failed against — the mirror of the false reassurance #755/#797 removed, out of the same channel. Each entry is now tagged (`rollback_error_kinds`, index-aligned beside an unchanged `rollback_errors`): `withheld` for the three composition withholds above and the two attachment refusals, `failed` for the eighteen producers where a restore was owed and did not land. It changes DRAWING, not counting — a withheld entry is still on the channel and still costs the clean claim, because the rollback was not clean — and a kind an older client or an older server does not know renders exactly as it did before |
