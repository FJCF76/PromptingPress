...
Note which routes a page in that state still leaves open, because the whole-composition posture is wider than it first reads: `update_component` validates the MERGED composition, so while a keyed-object list is stored, a patch to an **unrelated band on the same page** is refused too. That is the same accepted cost `#707`/`#600`/`#643`/`#744` each carry, not a lockout — the repair is one full `update_composition` (above), an editor-pane save, `restore_composition` to an earlier version, or `remove_component` on the offending band. `remove_component` and `reorder_components` run no composition validation at all, by design, so removing or moving a bad band always stays possible. `{}` and `[]` are indistinguishable once parsed and both count as the empty list, and `{"0": ..., "1": ...}` in order decodes to a PHP list before any validator sees it, so it is accepted; a *reordered* numeric object (`{"1": ..., "0": ...}`) does not and is refused.
