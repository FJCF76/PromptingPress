...
| `chrome` | `{version, chrome: {name ⇒ udc map}, corrupt, presets, presets_version, presets_unreadable}` | The site chrome styling container (`pp_site_udc`), which is how the header and footer are styled — one `udc` map per chrome component, in the same shape a band's takes. `version` is the concurrency baseline: pass it back as `update_site_option`'s `expected_version` (or leave it in the map you send) and a write that would overwrite someone else's newer edit is refused with `site_option_conflict` instead of clobbering it. `corrupt: true` means the stored row could not be read at all — no chrome styling is applied and every baselined write is refused until it is repaired or cleared, because a baseline cannot be checked against bytes nobody can parse. `{version: 0, chrome: {}, corrupt: false, presets: {}, presets_version: 0, presets_unreadable: []}` on a site with no chrome styling. **`presets_unreadable` names any stored preset row this engine could not read**, and a non-empty list blocks every preset write with `site_option_corrupt` until those rows are repaired or removed — so it is the first thing to check when `save_preset` refuses for a reason that has nothing to do with the preset you sent. **`presets` is the site's custom preset store (#1016), which shares this row and is written only by `save_preset` / `delete_preset`** — one entry per preset, each `{grain, udc}` plus an optional `description`. It carries its OWN baseline, `presets_version`, so a chrome write never stales a preset baseline and a preset write never stales a chrome one; pass it back as those verbs' `expected_version`. The two subtrees are preserved across each other's writes, including a chrome CLEAR, which removes chrome styling only. A chrome write that CARRIES `_presets` or `_presets_version` in its value is refused naming the preset verbs, rather than accepted and ignored: that value is real design, and accepting it silently would report a write that did not happen. Theme-shipped presets are not in this store and cannot be deleted or replaced by name. Separately from the baseline check, a write the database refuses is now reported as one: `update_site_option` returns `site_option_write_failed` rather than reporting success over a row that never changed (#981). The same is true of a CLEAR (`""` or `{}`): a removal the store refuses returns `site_option_write_failed` and the chrome styling is still there, while clearing a row that was already absent succeeds. A write whose value the option already holds still succeeds, and does not write twice. |
...
**A `--post_id=N` preflight fails closed on a corrupt page.** The composition it would freeze as the run's restore baseline has to be readable; if `_pp_composition` classifies `unexpected_shape` or `decode_error`, recording `[]` as the baseline would let a later `apply restore-composition` blank the page (#241). So the preflight records nothing and reports:

> `Could not read a valid pre-apply composition baseline for run token "<uuid>" (post N): the stored composition is <classification>. PREFLIGHT was not recorded, so both the action gate and the restore baseline stay fail-closed. Repair the post with ONE whole-composition write, which needs no preflight on a page in this state (#767): ...`

**Corrupt-page repair without a preflight (#767, ruling D-1).** ⚠️ **Narrows an enforcement gate in 1.16.14.** Because the preflight above refuses this page, requiring preflight coverage before repairing it made the repair unreachable from the CLI: `apply preflight` said "repair it first" and `action execute update_composition` said "preflight it first". The gate now admits the repair, under three conditions that must all hold:

1. the page's stored composition is **already** classified corrupt (`unexpected_shape` / `decode_error`) — read from the database, not the object cache, so a stale value cannot open the gate;
2. the action is one of the **two whole-composition verbs**, `update_composition` or `restore_composition`;
3. the incoming replacement still passes **full validation** before anything is written.

What that means concretely, stated as a narrowing rather than a feature: an operator who can already run `wp pp` on the server can now perform a fully-validated whole-composition write on a corrupt-classified page **without** first running `wp pp apply preflight`. Nothing else changes. It is not a general preflight bypass — a healthy page, a blank page, any other action, and any other command are all gated exactly as before, and a carve-out repair records **no** preflight coverage, so the next `add_component` on that same page in the same run is still refused for want of a preflight.

**The ruling has two consuming surfaces, and this section documents the CLI one.** Conditions 1 and 2 above live in a single shared predicate (`pp_corrupt_page_repair_carve_out`) precisely so the two surfaces cannot drift on which pages are repairable. The other consumer is the AI chat's batch refusal (#756, in 1.16.16): a proposal whose ONLY step is one of these two verbs, on a page in this same classification, is admitted through the #749 gate that used to refuse it. The condition each surface adds for itself is different — the CLI adds nothing, the chat adds "exactly one step" — and so is the gate each one lifts: the CLI's is described below, and the chat's under `pp_ai_execute_batch()` in AI_CONTEXT.md. The predicate deliberately knows nothing about the request, so capabilities, nonces and run tokens stay each surface's own.

Two gates lift on the CLI, and only for that case: preflight **coverage** (#96) and preflight **freshness** (#113). The freshness gate is not merely waived, it is unsatisfiable — its baseline is recorded *by* the preflight this page cannot pass. In its place the write takes a **live** CAS baseline, so `pp_update_composition()`'s in-lock compare-and-swap still refuses with `composition_conflict` if another writer repaired the page in the meantime; a carve-out admission never overwrites a composition that stopped being corrupt. Everything else holds: the run token, the `INSPECT`-first ordering, `pp_validate_action()` (including the whole-composition validation of the array you send), `restore_composition`'s #233 report-never-block contract and both its preconditions — `history_entry_not_restorable` on a preserved-bytes ring slot (#818) and `history_target_shifted` when a concurrent write moved what the selector names (#829) — and the per-post advisory lock.

```bash
wp pp operate inspect                                   # run token; no preflight needed
wp pp action execute update_composition --run-id=<uuid> \
  --params='{"post_id":234,"composition":[{"component":"hero","props":{"title":"Recovered"}}]}'
wp pp operate composition-history --post_id=234         # the unreadable bytes, preserved (#818)
```
...
`expected_version` is an **optional** param on all seven composition-mutating actions (`update_composition`, `add_component`, `remove_component`, `reorder_components`, `update_component`, `style_component`, `restore_composition`). Omit it and the write proceeds unconditionally (back-compat: new-page creation, the homepage seed, and legacy direct callers all skip the CAS). Supply it — the CLI agent path, the dashboard composition editor, and the AI chat path all do — and a concurrent write is rejected instead of silently clobbered. The **AI chat path supplies it mandatorily and fail-closed** (#404): the chat backend captures the page's `composition_version` when the model reads the page, and both `wp_ajax_pp_ai_execute` and `wp_ajax_pp_ai_execute_batch` REJECT a composition-mutating write with no baseline (`missing_expected_version`) before executing — the batch rejecting the whole batch before any step runs. A batch threads a per-`post_id` baseline map and chains each write's post-write version into the next mutating step on that page, so it never false-conflicts against its own writes. On `composition_conflict` the handler returns the structured envelope (code + `expected_version`/`current_version`) and the chat UI offers **Re-read & re-preview**, never a blind retry — and, since #704, never the **Ask the AI to fix it** affordance either: a conflict is not the model's to answer, so the batch response carries no `model_note` for it and nothing re-enters the conversation. Since #797 the card's own sentence is gated on `rollback_errors`: a conflict at a failing STEP rolled earlier steps back, so "Nothing was applied." is claimed only over an explicitly clean report, and a report with entries names what stayed dirty instead. Since #856 that naming survives the affordance: taking **Re-read & re-preview** leaves the card in the transcript with its button spent and renders the fresh proposal below it, rather than removing the only copy of the report — a card whose report was clean or unreadable is still removed. The dashboard editor keys on the same structured `composition_conflict` code to prompt a reload; the version it sends is refreshed from each successful save so a run's own sequential edits never false-conflict.

**A composition the encoder cannot represent is refused, not stored as an empty page (#941).** ⚠️ **New refusal class in 1.19.11**, on the same writer. `pp_update_composition()` encodes the composition to JSON before it takes the lock, and that encode used to be unchecked: on failure `wp_slash(false)` is `false`, so `update_post_meta()` stored an **empty value**, the call returned `true`, `ok: true` came back with both freshness markers bumped, and the page then read back as a perfectly healthy blank one — an empty `_pp_composition` row is a legitimate "no composition yet" state, so `wp pp check page` reported no error and `findings: []` certified it. An accepted write emptied the page and called the result clean. The write now **REFUSES**, before the lock is acquired, with `composition_not_encodable`:

> `The composition for post 231 could not be encoded as JSON (Inf and NaN cannot be JSON encoded), so the write was REFUSED and nothing was written: post 231 still stores the composition it stored before this call, and its version and content markers are unchanged. Retrying with the same data will fail the same way — this is not a lock or a conflict. Send a composition that JSON can represent (no values nested past the encoder depth limit, no self-references, no INF/NAN floats, no resources) and write again. Earlier states of this page are recorded in its history ring — list them with `wp pp operate composition-history --post_id=231`. [composition_not_encodable]`

The last sentence appears only when that page's ring actually holds entries; a page with fewer than two accepted writes has none, and the message then says nothing about a ring rather than pointing you at an empty one. **Nothing is narrowed for any caller that could reach this before.** The trigger set is narrow and worth stating exactly, because this path encodes with `wp_json_encode()` and that function coerces more than a bare `json_encode()` does: a `false` return means nesting past the encoder's depth limit, a self-referential array, a non-finite float (`INF`/`-INF`/`NAN`), or a value of a type JSON cannot represent (a resource). **Malformed UTF-8 is NOT in the set** — core's `_wp_json_sanity_check()` rewrites strings and re-encodes, so that class succeeds. And none of the four can be authored through a validated call today: whole-composition validation refuses those shapes earlier and louder (`unknown_prop`, `invalid_prop_value`, `invalid_composition`), and `restore_composition` replays bytes that already decoded. So this is a **backstop over in-process writers** that hand the one composition writer a PHP array nothing validated, not a gate a CLI or chat call can currently trip. It is the composition-side counterpart of the ring-encode guard below, which took the opposite posture on the same signal for a stated reason: a failed ring push costs a caller its undo point, while this write IS the loss.

**Which band blocked the write (#642).** `create_page` and `update_composition` validate the WHOLE composition, so a rejection from either routinely comes from a band the caller never named. `update_component` did too until #1007; it now validates only the band it targets, so its rejections name the band you asked about — except for the CROSS-ITEM rules (`duplicate_component_id`, `duplicate_band_id`), which are properties of the page and still refuse from any band, because the writer re-serializes the whole composition and accepting the write would store the collision again. A rejected envelope therefore carries `index` beside `error_code` (the composition offset of the owning band, or `null` when no single band owns it), and the message names the same band up front:

> `Component 1 ("logos") prop "items" item 0 field "image_id" must be a number; got array. [invalid_prop_value]`

Before this, two same-type bands produced byte-identical rejections, so an agent "fixed" its own payload, re-submitted, and got the same string back forever. Two rejection families already name the component in their own words — `Unknown component: "x".` and the site-chrome message below — so those are prefixed with the offset instead (`Component 1: Unknown component: "nope".`); the four structural messages already spell `Item N` and are left alone. `null` means no fabricated locator, not "band 0": it is what a cross-item rule (`duplicate_component_id`), a param-shape error, a failed precondition, a rejection on a band the caller named itself (`style_component`'s own validator, `index_out_of_bounds`), and `add_component` (which validates only the item it adds, so its offset is not a page offset) all report. The field is the authoritative locator — it and the message are rendered from the one stamped offset, so they cannot drift, but a message also reflects author-supplied bytes (a component name, a prop key) and a composition authored with `Component 9 ("x")` inside one of those puts a second band-shaped phrase in the sentence. `wp pp action execute` emits the whole envelope, as does each step of a chat batch. Three paths convey a rejection as its message alone, so there the band is in the words and not in a field: `wp pp action preview` (which renders a rejection as `{ok, error}`), the chat preview, and the chat single-execute error payload. In a batch the failed step's locator is honest or absent, never stale (#712). A failed batch rolls every earlier step back, so an offset computed against the mid-batch composition would address a band the rollback moved; the executor therefore NULLS `index` on the failed step when an earlier step in the same batch wrote that page's composition (any of the SEVEN actions that declare `mutates_composition` — `style_component` included, because a style write is a composition write — or a `create_page` whose new page the rollback deletes). When no earlier step wrote that page, the batch moved no bands there and the locator is KEPT — it still addresses the offending band in the restored composition. Not re-anchored, because no inverse map exists in general: `update_composition` and `restore_composition` replace the list wholesale, `add_component` can create a band that exists only mid-batch, and a create_page target is deleted outright, so a re-anchored offset could not be proven to point at the offender. The failed step's MESSAGE is left verbatim and still quotes mid-batch band prose — it belongs to the validator that produced it, and that half predates #642 — so re-read the page before acting on the words. **The rejection examples quoted in the rest of this section show the message from the component label onward, without that band prefix, so the rule each one illustrates stays legible; a real write rejection carries it.** The reporting surfaces are unaffected — `wp pp check page`, `validate site` and restore/rollback `findings` carry the offset as their own field beside the message (`[unknown_prop] index 1: ...`) and their message text is unchanged.
...
The shape, on the advisory species that replaced `inert_slot` when the style-slot engine
retired at #1101. Same channel, same envelope, same claim — a value the engine stored and
reported as applied that paints nothing — on the engine that is live:
...
{ "ok": true, "action": "update_composition", "composition_version": 2,
  "findings": [
    { "type": "udc_band_value_shadowed_by_role_default", "severity": "warning", "index": 0,
      "message": "Component \"grid\": the \"color\" you set on the whole band does not reach card-bullets, card-link, card-text, card-title, empty, eyebrow, heading-accent, step-number, subheading, because those roles' own default for it wins over inheritance. Set it on those roles directly (eyebrow, step-number ship their own fill and text colour, so they keep that designed pair; to recolour one, set its background.fill as well)." }
  ] }
```

**Since #993 the key is not composition-only:** `update_site_option` on `pp_site_udc` — the chrome map — carries `findings` too, in the same shape and on the same channel, so the minting and preset-skip disclosures a band write reports are now reported for chrome as well. Chrome entries carry `index: null`, because a chrome entry has no band offset and claiming one would be a fabricated locator. **Since #1016 `save_preset` and `delete_preset` carry it too**, on that same channel and with the same `index: null`: a preset is referenced FROM chrome, so editing one changes what chrome paints without any chrome write happening, and the write that caused it is the one that should say so. Stated limit: that disclosure is derived from the CHROME container only. A preset write that changes what BANDS paint reports nothing, because the composition disclosures are assembled per composition write.

`wp pp action execute` and `wp pp operate patch` print the whole envelope, so this arrives with no flag and no second command. Same shape as the reporting surfaces (`type` / `severity` / `message` / `index`) and the same two engines, so `wp pp check page --post_id=N` says the same things — the difference is that a write no longer stays quiet about them. `severity: error` findings ride along too, because the band-scoped actions validate only what they touch (`update_component` validates the band it TARGETS, `add_component` only the item it adds) and so legitimately accept a write onto a page whose other bands are stale; the report names those bands by `index` rather than leaving you to discover them on the next whole-composition edit. It is **report-only**: computed after the write lands, on success only, from the stored bytes — it never blocks, alters or reorders anything, and a REJECTED envelope carries no `findings` key at all and still returns exactly one message. Long reports are bounded at 100 findings plus one `findings_truncated` entry stating the true total and naming the page to run `wp pp check page --post_id=N` against for the rest. That entry carries the true total as a `total` integer as well as in its message, so a consumer that renders a count reads `total` rather than counting the array it received; the key appears only on a truncation entry, so its absence means nothing was omitted. Since #654 that bound reaches `restore_composition`'s own `findings` (#233) too — preview and execute alike — and the run-scoped rollback, which bounds per reverted post so each tail names its own page. `wp pp check page` itself is the deliberate exception and reports completely: it is the command the tail points at, and a breadcrumb that returned the same first 100 findings would be no breadcrumb at all. Two limits worth knowing. The cap is a flat per-report count and findings arrive errors-then-advisories, so on a composition carrying more than 100 error-severity findings the advisories — `inert_slot` included — fall behind the tail; the tail says the report is incomplete, and a page in that state is telling you something louder than an advisory anyway. And the cap bounds the COUNT, not bytes. A message reflects stored names verbatim, and `duplicate_component_id` names every colliding index in a SINGLE entry, so its length grows with the band count and 100 entries is not 100 short strings. This is reachable without any raw meta write: `add_component` validates only the item it adds, so repeated calls carrying the same authored `props.id` are all accepted, and every later accepted write on that page then carries an O(N) duplicate-id message (plus its advisory twin). Give each component a unique authored `id` and the report stays small. Since #647/#649 the reflected KEYS and VALUES in a message are bounded (a rejected value at 100 characters, an undeclared prop key and an `items[]` key locator at 64); the stored component NAME is still reflected verbatim, which is the half that makes this message grow with the band count.

**One case reports nothing, and says so (D1 Addendum #2).** Building the report costs roughly 28 MB of transient memory per MB of stored composition, and it runs *after* the write has landed — so on a large enough page it could exhaust `memory_limit` and kill the response to a change that already happened (no envelope, no touched-post record for `apply restore-composition`, no refreshed CAS baseline). Above **1,048,576 bytes** of stored composition JSON the engines are not run at all and the composition report is exactly one entry instead (one *composition* finding — the `history_not_recorded` entry below is about the write, not the composition, and can still lead the list, so match on `type` rather than expecting a single-element array):
...
**One case reports that the write cannot be undone (#821).** Every composition write pushes the state it replaces onto the page's history ring first, which is what makes the change reversible. When that entry cannot be JSON-encoded — reachable today through nesting depth, since wrapping a stored composition inside a ring entry inside the ring puts it two levels deeper than it sat when it was read back — the ring write is skipped and the composition write proceeds anyway. The ring is left exactly as it was, so nothing already in it is lost, but this write added no entry to it. The envelope leads with:

```json
{ "type": "history_not_recorded", "severity": "warning", "index": null,
  "message": "This write landed, but the state it replaced could not be recorded in the page history, so THIS WRITE HAS NO UNDO POINT. The history ring was left intact and still holds its earlier entries — run `wp pp operate composition-history --post_id=19` to see what can still be restored." }
```

It is the FIRST entry, and it sits OUTSIDE the 100-finding budget — deliberately, for the same reason `findings_truncated` is itself the 101st entry: the budget bounds the report about the *composition*, and this is a report about the *write*, so a page pathological enough to truncate must not be able to push the "your change is irreversible" line off the end. `total` on a truncation tail therefore still counts composition findings only. Any accepted composition write can carry it, including `restore_composition` (which owns its own `findings` key) and the run-scoped rollback (which carries it per reverted post, and warns about it separately from the validation-rule warning, since a page with only this entry breaks no rule). The rolled-back-batch caveat below applies to it exactly as it applies to the rest of the report: after `rolled_back: true` the entry describes a step's write that was reverted, so it names a change that no longer exists.

**Where you will and will not SEE it.** It is on the envelope of every accepted composition write, so `wp pp action execute`, `wp pp operate patch` and the run-scoped rollback all print it. In the chat only the undo card renders it; the post-apply card reads `validation`, not `findings`, so an ordinary chat edit that loses its undo point shows nothing in the UI, and the dashboard editor does not surface it either. On those surfaces the server log line (`history ring NOT updated`) and the CLI envelope are the record. The DB-layer half of the same failure is deliberately still silent: `update_post_meta()` returns `false` both for a genuine failure and for a value that did not change, so blocking or reporting on that ambiguity waits on a compare-first read.

Read `findings_skipped` as "not measured", never as "clean" — an empty `findings` list means the write was diagnosed and found nothing, and this is the opposite claim. `wp pp check page` has no such limit and will report the page in full. The threshold is a fixed constant with no filter or option. A realistic six-band composition stores about 5 KB, so ordinary pages sit orders of magnitude under it.

The rolled-back-batch caveat applies here and, since #712, ONLY here. A batch step's `findings` are built when that step succeeds, before a LATER step's failure rolls the whole batch back — so a failed batch returns per-step reports, and `index` values inside them, describing compositions that no longer exist. Re-read the page after a rolled-back batch rather than acting on them. The failed STEP's own `index` is the one locator that no longer carries this caveat: the executor nulls it when the rollback invalidates it (see the `index` section above).
...
**What breaks, and the fix.** Any caller sending an object-shaped `composition` param is now rejected instead of silently destroying bands — send a JSON array. Nothing is coerced or reindexed on your behalf (ruling D-A: reject, never coerce), and nothing stored is migrated. The error `unexpected_shape` is deliberately the read path's own word for this state, so a refused write and `wp pp check page` / `wp pp operate inspect` / `inspect-composition` all name it identically. This rejection carries **no `index`** — the problem is the container, not any band, so no band locator is invented. Two shapes are indistinguishable after `json_decode` and remain accepted: `{"0": ..., "1": ...}` (the keys ARE `0..n-1`, so PHP hands back a list — and key and position agree, so nothing is dropped) and `{}` (identical to `[]`). A page that already stores the object shape is **not** migrated: `wp pp check page` and `wp pp operate inspect-composition` both report it as corrupt, and one full `update_composition` write with a JSON array repairs it. `restore_composition` used to be the deliberate exception (#233) — a JSON object decodes to a PHP array, so it was snapshotted as a composition and replayed verbatim. **Since 1.17.1 it is not (#841):** an object-shaped prior is preserved as bytes and refused with `history_entry_not_restorable`, like the SCALAR sub-case (a stored `null`, `5`, `"text"`) has been since 1.16.13 (#818), and like a `decode_error` page. Replaying an object either rewrote a healthy page back into a corrupt one and reported success, or — when the object was a single band, `{"component":"hero",…}` — crashed the command outright. #233 is not weakened by that: it says restore is never blocked by current VALIDATION RULES, and every slot that carries a list-shaped composition still replays verbatim and still reports, however illegal today's rules find its contents. This refusal says the slot holds no composition at all. Rings written before 1.17.1 hold slots filed the old way; those are reclassified when the ring is read, so they refuse instead of crashing (see "Composition history & restore" below for the byte-exactness caveat on those older slots). **Send either repair as a SINGLE action, or as a chat proposal whose ONLY step is that action (#756).** A batch is refused before step 1 when any page it names has an unreadable stored composition, and the chat client routes every proposal through the batch endpoint — so until #756 the repair sent that way came back refused with the very code it was meant to clear. Ruling D-1 now admits exactly one batch shape through that refusal: a lone `update_composition` / `restore_composition` step on a page already classified `unexpected_shape`/`decode_error`. Put a second step beside it and the whole proposal is refused again; aim it at a page that is not corrupt and there was never a refusal to lift. Only the batch preflight is waived — capabilities, the CAS baseline and full validation of what you send all still run. The single-step path takes no rollback snapshot and so is never refused by that gate at all: `wp pp action execute update_composition` / `wp pp action execute restore_composition`, or the dashboard editor. Those two verbs are the entire corrupt-page carve-out (ruling D-1) and the CLI admits them **without a covering preflight** on this classification only — see "Corrupt-page repair without a preflight" under `wp pp apply preflight` above. `pp_patch_composition()` / `wp pp operate patch` is **not** a repair route: it is refused on a corrupt page by the `composition_required` precondition (#748), and a field selector cannot reshape a container anyway.
...
> `Duplicate component id "pricing" on items 0, 2. Component ids must be unique within a composition so update/remove/style can target one component. This refuses an edit to ANY band, including bands that are not colliding, because a write re-serializes the whole composition and would store the collision again — repair the ids through update_composition. [duplicate_component_id]`

That trailing clause was added at #1007 and is the one place the page-level/per-item split is stated to the operator. `update_component` narrowed to the band it targets in that change, so a stale PROP on another band no longer blocks — but a duplicate id is a property of the whole page, and the writer re-serializes every band, so this one still refuses from anywhere. `duplicate_band_id` (the v2 id that scopes a band's emitted CSS) carries the same clause for the same reason.
...
**Retired props (#1007).** A prop a component USED to declare and no longer does is refused with its own code, `retired_prop`, not the generic `unknown_prop`, and the message names the v2 surface that replaced it plus the way to clear the stored key:
...
> `Component "cta" has no prop "not_a_real_prop". Available props: id, title, title_accent, eyebrow, body, button_text, button_url, button2_text, button2_url, layout [unknown_prop]`

The source of truth is the component's full schema `props`, not the narrower schema-derived scalar-patch set (`pp_get_component_fields()`, which exposes only scalar props for `operate patch`), so a real prop like `grid.items` or `section.panel_items` is accepted while a misspelled or invented key is not. (`cta.theme` and `cta.background_image` were this row's examples until #1026 retired both; they now refuse with `retired_prop`, a different code with a different answer.) — closing the "phantom field" hole where an unknown key would persist behind an `ok:true` while the renderer silently ignored it. Unlike template-owned chrome and duplicate ids, this rule has no composition-smell counterpart. A stored composition that already carries an unknown prop (from a legacy write or a raw non-action path) is still reported, as an ERROR-severity finding rather than an advisory smell: `restore_composition` returns it in `findings` (never blocking undo, #233), and since #622 `wp pp check page` and `wp pp validate site` report it too — a stale page no longer reads as clean on the surfaces you query first.
...
**Schema-typed prop values (#507).** Beyond the *key* being known, each prop's *value* is checked against its declared schema `type`, so an accepted write renders as authored instead of the renderer emitting `Array` (with a PHP warning) behind an `ok:true`. A prop declared `type: "string"` rejects anything that is not a string — including a non-string SCALAR (`42`, `3.14`, `true`, `false`), narrowed in #707; `type: "number"` rejects a non-numeric; `type: "array"` rejects a scalar, and since #738 a JSON object where a list belongs too (see the #738 paragraph below); `type: "object"` rejects a scalar too since #744 (built ahead of any shipped declaration — no composable schema declares a top-level object prop today — so the two depths cannot disagree the way they did before #744); and an object-item array (one declaring `item_type: "object"`, e.g. `grid.items`) rejects any entry that is not an object. Each check has a per-type "unset" sentinel (`null`, `""`, and — for arrays — `[]`, which `{}` is indistinguishable from once parsed) that preserves the prop's default, so an omitted value is never a rejection.
...
> `Component "grid" prop "card_emphasis" must be one of: featured, uniform; got "neon". [invalid_prop_value]`
...
Reject, never coerce: a scalar is not wrapped in a one-element list, and nothing stored is migrated — the render-side `is_array()` guards still turn a stored scalar into an empty list rather than a fatal. What remains deliberately uncovered is a container's **contents**: nothing constrains an item `object` field's slot values. The container rule itself decides only "container or scalar?" — a JSON *list* handed to an `object` field satisfies it (PHP decodes both JSON containers to an array), which is why the two shape rules below exist as separate second stages.

**A declared list must be a JSON ARRAY, not an object with keys (#738).** ⚠️ **Breaking change in 1.18.6.** `type: "array"` now means a JSON *list*, not merely "a container". A JSON object where a list belongs is rejected at both depths, through a shared predicate beside the one above (`_pp_schema_list_value_is_valid()`):

> `Component 0 ("grid") prop "items" must be a list, but this one is a JSON object (2 entries). Send it as an array ([...]), not an object with keys. [invalid_prop_value]`

Until #738 it was accepted: a schema-clean `update_composition` carrying `"items": {"first": {"title": "Card one"}, "second": {"title": "Card two"}}` returned `ok:true` with no findings, persisted as written, and the public page then **500'd** — `components/grid/grid.php` derived a card's step ordinal with `(string) ($index + 1)`, and a string key plus an int is a PHP `TypeError` that `templates/composition.php` does not catch. `??` short-circuits, so a keyed object whose every entry carried `number` rendered fine and the page died the moment one field was deleted. The container rule above could not catch it (a JSON list and a JSON object both decode to a PHP array, which is exactly what it says it does not decide) and the `item_type: "object"` rule could not either (every entry was a valid object).

The narrowing covers the nine top-level `array` props — `faq.items`, `grid.items`, `logos.items`, `section.body_items`, `section.panel_items`, `stats.items`, `table.headers`, `table.rows`, `testimonials.items` — and the one nested `array` field, `grid.items[].bullets`. `type: "object"` is closed by its own rule, below.

**Fix:** re-send the prop as an array. `wp pp action execute update_composition --post_id=<id> --composition='[{"component":"grid","props":{"items":[{"title":"Card one"},{"title":"Card two"}]}}]'`. Nothing is migrated for you, and `restore_composition` is not blocked by this rule (or any other, #233) — it restores verbatim and reports the shape in `findings`.

Note which routes a page in that state leaves open. Until #1007 `update_component` validated the MERGED composition, so a patch to an **unrelated band on the same page** was refused too; that cost is gone. `update_component` now validates the band it targets, so an unrelated band is editable while the keyed-object list is still stored, and the stored problem is reported on the accepted envelope's `findings` instead. The band that HOLDS it still refuses its own edits until it is repaired, and the repair routes are unchanged: one full `update_composition` (above), an editor-pane save, `restore_composition` to an earlier version, or `remove_component` on the offending band. `remove_component` and `reorder_components` run no composition validation at all, by design, so removing or moving a bad band always stays possible. `{}` and `[]` are indistinguishable once parsed and both count as the empty list, and `{"0": ..., "1": ...}` in order decodes to a PHP list before any validator sees it, so it is accepted; a *reordered* numeric object (`{"1": ..., "0": ...}`) does not and is refused.
...
**The reflected key is bounded (#649).** An `items` map is only string-keyed when it came from a raw write, a history-ring snapshot, or a `restore_composition` that replayed one — never-validated data, which is exactly what these diagnostics are pointed at. (Before #738 an author could send one through an ordinary write too; that route is closed, so the remaining sources are all paths that deliberately bypass write validation.) So the key goes through the same helper the rejected *values* in these messages use: control and format characters are stripped, and a key longer than 100 characters is cut and marked with `...`. Every well-formed key is byte-identical — `item key "aa"` and `item key "0"` above read exactly as they always did. What changed is that a key carrying an ANSI escape or a bidi override can no longer render as one, on any surface that carries the message: the terminal already stripped it, and now the action envelope, the editor save response and the chat reply do too. Quoting is unchanged, so a key containing a double quote still renders ambiguously (`item key "a"b"`) — that is a known limitation of this grammar, not a bounding gap.
...
⚠️ **Since #724 the right-hand column is a STORED-STATE column only.** A write can no longer produce any of those four messages: an object-shaped composition is refused at the container with `unexpected_shape` before the per-band rules run, so a rejected `create_page` / `update_composition` never names a band by key. The key forms remain the honest answer where an object-shaped composition is still *read* rather than written — the run-scoped rollback report over raw history-ring snapshots, which admits that shape. `restore_composition` left that list in 1.17.1 (#841): the history ring now asks the same list question at both ends, so an object-shaped slot is reclassified to preserved bytes before any reader sees it and the restore is refused rather than replayed — see "Composition history & restore" below. The `items[]`-depth `item key "N"` forms above are unaffected and stay fully live on ordinary list-shaped writes: an `items` map inside a well-formed band is a different container from the composition itself.
...
| `token_override_validity` | always; reports only when a stored override was dropped, or when the token registry could not be read (the `:root` token block is not page-scoped) | no (`severity: warning`) |
| `udc_value_cannot_take_effect` | always for site chrome; a page's bands when `--post_id` is given — bands with a band-level map, then (since #1117) bands styled only at card grain. Reports only when a stored `udc` value is not painted | no (`severity: warning`) |
| `screenshot_readiness` | always | no (`severity: warning`) |

`ok` is `true` only when no **error-grade** check failed. Rows with `severity: warning` (nav readiness, token-override validity, unpainted `udc` values, screenshot readiness, non-overlapping drift) surface a problem without blocking.

**Stored `udc` values that are not painted (#981).** A value can be in a band's `udc` map, or in `pp_site_udc`, and still not reach the page: its `@token` reference resolves to nothing, the stored value no longer satisfies its parameter's grammar, the parameter or group is no longer declared, a breakpoint key is unknown, or a single-valued parameter holds a map. The write gate refuses most of those shapes, but it is not the only door — a raw meta write, a composition written before a rule existed, a renamed parameter, and `restore_composition` (which reports findings without blocking) all reach the emitter directly. This check is the account of what stopped painting.

It reads the EMITTER's own ledger rather than re-deriving the rules, so what it names is what the page omits. Rows are `udc_value_cannot_take_effect:<hash>`, configuration-class and acknowledgeable, with the styling action as the next action; the hash covers the location AND the reason, so acknowledging one stale value does not silence a different problem later at the same role and parameter. Two rows are shaped differently: more findings than fit are summarised by one `udc_value_cannot_take_effect:overflow` row, whose count is a FLOOR rather than a total — it reads "At least N more", because the walk stops at a band bound and the per-compile ledger is capped, so the true number can be higher; and a page with more styled bands than the walk reads reports `udc_value_cannot_take_effect:bands_truncated:<post_id>` so a partial list never reads as a complete one. The page walk is two passes with a budget each (#1117): bands with a band-level map first, then bands styled only at card grain, so band-map rows are listed first and the truncation row fires when either pass runs out. A healthy page reports nothing, however many bands it has. Site chrome is checked unconditionally, because chrome renders on every page and there is no context in which skipping it would be right. (It used to say this was chrome's only channel, because a chrome write returned no `findings` at all. Since #993 it does carry them, and the two answer different questions: the envelope reports what one WRITE did, this reports what the STORED state still does.)

**Token-override validity (#965).** Stored design-token overrides are re-validated against the type `assets/css/base.css` declares for each one before the theme emits them. A row that does not validate is left out of the `:root` block — the rest still emit — and reported here, one finding per dropped token up to ten, so a token that stopped painting is always accounted for. Finding keys are `token_override_validity:<token>:invalid_value` for a declared token whose stored value no longer validates, and `token_override_validity:unregistered:<hash>` for an override naming a token the theme does not declare; both are configuration-class and acknowledgeable, with `update_design_token` / `reset_design_token` as the next action. Two rows are shaped differently: a run of more than ten dropped tokens is summarized by a single `token_override_validity:overflow` row, and if the token registry itself cannot be read (`assets/css/base.css` missing or unparseable) the check reports one **integrity**-class `token_override_validity:registry_unavailable` row about the theme file instead of one finding per token — the overrides are intact in that case, so it never advises clearing them.
...
Design-token writes have always been reversible (snapshot at preflight, `restore`). Composition writes — the page content itself — now match that parity. Every composition write (`update_composition`, `add_component`, `remove_component`, `reorder_components`, `update_component`, and `restore_composition` itself) pushes the **prior** state onto a bounded per-post history ring (last 10 entries) before overwriting. Restore reads that ring. The one exception is a ring entry that cannot be JSON-encoded: the push is skipped, the write still lands, and it says so with a `history_not_recorded` finding (#821, above) — the ring keeps its earlier entries, but that particular change has no slot to step back to.

**A ring slot holds one of two things (#818).** Normally a **composition snapshot** — the prior components, replayable by `restore_composition`. But when the bytes a write replaces did not decode to a composition (a `decode_error` page, either sub-case of `unexpected_shape` — a valid-JSON scalar, or a valid-JSON object — or, since 1.19.10, a valid-JSON **list whose entries are not components**), the slot holds those **preserved bytes** instead. Before 1.16.13 such a write pushed *nothing*, so repairing a corrupt page destroyed the only recoverable copy of what was there — and the documented repair is exactly one full `update_composition` write, which made the recovery path the destructive one. The bytes are now kept, readable through `wp pp operate composition-history`, and refused by `restore_composition` (they are not a composition, so there is nothing to replay). The trade, stated: a preserved-bytes slot consumes a ring slot like any other, so a page corrupted and repaired repeatedly evicts good snapshots faster.

**The object case joined that list in 1.17.1 (#841), and rings written earlier are migrated on read.** A JSON object decodes to a PHP associative array, so before 1.17.1 an object-shaped prior passed the push's array test and was filed as a replayable snapshot: `wp pp operate composition-history` reported it `restorable: true` with a component count taken off the object's keys, and selecting it crashed the command instead of restoring anything. New writes preserve it as bytes like the other classes. "Object" here is narrower than it sounds, exactly as it is on the write path above: `{"0": ..., "1": ...}` decodes to a genuine PHP list (the keys ARE `0..n-1`), so nothing below the decode can tell it from an authored array and it is still snapshotted and still replays — only a genuinely string- or gap-keyed object reaches the refusal, and `[]` / `{}` are unaffected. A slot filed the old way is reclassified when the ring is read, so it now lists as `restorable: false` and is refused with `history_entry_not_restorable` rather than crashing — with one honest caveat: for those pre-1.17.1 slots the `raw` / `raw_base64` bytes are the ring's decoded copy re-encoded, not the exact bytes the page stored, because the exact bytes were never kept for that class. Slots written by 1.17.1 and later are byte-exact for every class.

**The list-of-non-components case joined it in 1.19.10 (#842), on the same terms.** #841 fixed the CONTAINER test; this is the ELEMENT test one level down. A prior whose container is a perfectly good JSON list but whose entries are not components — `["a","b"]`, or a list holding `{"component":"x","props":"str"}` where `props` must be a container — passed the container test, was filed as a replayable snapshot, listed as `restorable: true` with a component count taken off things that are not components, and crashed the command on selection with the same uncaught error the object case produced. New writes preserve it as bytes; rings written earlier are reclassified on read with the same re-encode caveat as the pre-1.17.1 slots above.

**What makes this class different from the other three: the page is not corrupt.** `wp pp check page` and `wp pp operate inspect-composition` report it healthy, because the classifier judges the container and this container is fine. So none of the corrupt-page machinery engages, and this is the one unrestorable slot you can meet on a page with nothing else wrong with it. That is deliberate: the refusal answers "can this slot be replayed?", not "is this page corrupt?", and turning it into a corruption class would coin a fourth spelling for a state the diagnostics already have words for.

**It is a replayability judgment, not a validity one, and the distinction is load-bearing (#233).** Restore is never blocked by current VALIDATION RULES, so the test is exactly "can the composition writer run over these entries at all", never "is this composition legal". An entry missing its `component` key is invalid and still restores, reporting the problem in `findings`; a bare list entry (`[["a","b"]]`) is nonsense to every renderer and still restores. Only the shapes that made the writer raise an uncaught error changed sides.

The precise boundary, since it is narrower than "not a valid composition" and narrower than the summary above: an entry must decode to an **array** (a JSON object or a JSON list), and its `props` — if present and not `null` — must decode to an array too. A missing `props`, a `null` `props`, `[]`, and `{}` are all unaffected and still replay. Deliberately NOT tested: whether the entry has a `component` key, and whether `props` is an object rather than a list. Both are validity questions, and #233 reserves those for the `findings` report — so `[{"props":{}}]` and `[{"component":"hero","props":["x"]}]` still restore, and still get told what is wrong with them. The refused set is the shapes where a container is required and a scalar sits instead. That is deliberately a little WIDER than the set that actually crashes: `false` and `null` as entries, and a `props` of `false`, do not crash the writer, but it rewrites them into a band that was never stored, so they are refused with the family they belong to rather than split off by which ones happen to raise.
...
Errors: `no_history` (the page has no recorded prior state), `history_out_of_bounds` (selector past the ring), `history_entry_not_restorable` (the selected slot holds preserved bytes, not a composition — read them with `wp pp operate composition-history --post_id=<id>` and select an earlier entry), `history_target_shifted` (another writer recorded a state on the page between your selection and the write, so your selector no longer names the entry you chose — re-read the ring and select again), `composition_conflict` (stale `expected_version`).
...
**Two preconditions can still refuse a restore, and neither is a validation veto. The first (#818).** If the selected ring slot holds *preserved bytes* rather than a composition snapshot, there is no composition to replay and the restore is refused with `history_entry_not_restorable`. That is a precondition failure of the same species as `no_history` and `history_out_of_bounds` — a statement about which slot you selected, not a judgment on its contents. Every slot that carries a composition still restores verbatim and still reports, however illegal today's rules find it. The refusal names the byte count and the command that prints the bytes; step to an earlier entry to roll the page back.

**The second: the selector is confirmed at write time, so `ok: true` means the snapshot you addressed (#829).** `steps_back` and `history_index` are resolved against the ring you read, and then confirmed against the authoritative ring inside the write lock. If another writer (a CLI action, the dashboard editor, the AI chat) recorded a state on the page in between, your selector no longer names the entry you chose, and the restore is refused with `history_target_shifted` — nothing is written. This matters most for `steps_back`, which is **relative**: it counts backwards from the newest entry, so *every* concurrent write changes what it means. A `history_index` taken from a current listing is stable until the ring evicts, which is why the refusal points you back at `wp pp operate composition-history`. Like the refusal above, this is a precondition failure — a statement about which slot your selector names, not a judgment on its contents.

One honest limit: the confirmation reads the ring straight from the database, and when that read is not possible (no usable database handle) or fails outright, it falls back to the cached ring rather than treating an unanswerable question as a shift — returning "empty" on a failed read is how a ring gets wiped. So the guarantee holds whenever the authoritative read succeeds, and a database blip degrades to the older behavior instead of refusing every restore on the site. The fallback logs a line.

Two things follow from the report-never-block contract, and both are visible in the result:
...
The chat's undo card styles each finding from its `severity` (#622): an error renders as an error, a warning as an advisory. The card's heading stays a warning that says "Restored" — the restore itself succeeded and is never blocked (#233). Since #655 the card also renders the locator in the CLI's own idiom (`[unknown_prop] index 0: message`, this section's format minus the list bullet), and it picks the handful of rows it shows inline BAND-AWARE: the first finding of each distinct `index`, with the rest behind the "Show N more" disclosure. Post-#621 a single broken band emits several findings, so a first-N-by-position rule filled every visible row from band 0 and hid every other affected band. A finding with no locator is its own group and prints `[type]: message`, as here. The `findings_truncated` tail renders outside the disclosure, so the `wp pp check page --post_id=N` command it names is visible without opening anything.

**The card's rows are LENGTH-bounded, and the CLI's are not (#793).** The count bound above is not a byte bound — a finding message reflects stored component names verbatim, and `duplicate_component_id` names every colliding index in a single entry — so on a pathological page one row can be arbitrarily long. Since 1.17.9 the chat card cuts each server-supplied span at 4096 characters and marks it with `...`: the locator and the message are bounded separately, so an oversized `type` cannot cut the message away. The bound is the client's, applied where the row is drawn; the payload on the wire is unchanged, and `wp pp check page --post_id=N` still prints the message whole. If a card row ends in `...`, that command is where the rest is.
...
⚠️ **Breaking change in 1.17.1 (#841):** an **object-shaped** ring slot changed sides. It used to list `restorable: true` with an integer `components` (a count of the object's keys); it now lists `restorable: false`, `components: null` and the four `raw_*` fields, and `restore_composition` refuses it with `history_entry_not_restorable` instead of replaying it. A call that returned `ok: true` for such a slot now returns `ok: false` — deliberately: replaying one either rewrote a healthy page back into a corrupt one while reporting success, or crashed the command outright. List-shaped slots, `[]`, `{}` and the folded `{"0":…,"1":…}` are unaffected.

⚠️ **Breaking change in 1.19.10 (#842):** a ring slot holding a **list whose entries are not components** changed sides the same way, and it is the sharper of the two because the page it sits on is *healthy*. Such a slot used to list `restorable: true` with an integer `components` counted off entries that are not components; it now lists `restorable: false`, `components: null` and the four `raw_*` fields, and `restore_composition` refuses it with `history_entry_not_restorable`. A call that returned `ok: true` for such a slot now returns `ok: false` — but it never actually restored anything: it raised an uncaught error and took the request down, on the CLI and in the chat's undo alike. Two shapes reach this: an entry that is not an array (`["a","b"]`), and an entry whose `props` is present, not `null` and not an array (`{"component":"x","props":"str"}`). "Array" means either a JSON object or a JSON list: a `props` written as a list is nonsense to every renderer but still replays, because that is the findings report's verdict to give, not this refusal's. Everything else is unaffected — a missing `props`, a `null` `props`, `[]`, `{}`, and any list of well-formed components. Entries that are merely INVALID still restore and still report (#233); what changed sides is the set where a container is required and a scalar sits instead — which is slightly wider than the set that literally crashed, since `false` and `null` entries and a `props` of `false` are rewritten rather than raised on, and are refused with the rest of their family.
...
**`raw_base64` is the recovery channel; `raw` is for reading.** The command's JSON sink encodes with `JSON_INVALID_UTF8_SUBSTITUTE`, and malformed UTF-8 is one of the corruptions that lands a page in `decode_error` — so `raw` can come back with those bytes *substituted*. `raw_base64` is pure ASCII and survives untouched; check what you saved against `raw_sha256`, which is taken over the bytes this command read. Nothing is truncated: a truncated recovery is not a recovery.

Two row classes are exceptions to "the true bytes", and both are rows an older release mis-filed as replayable snapshots: an object-shaped prior written before 1.17.1 (#841), and a list-shaped prior whose entries are not components written before 1.19.10 (#842). Each is reclassified on read, so its `raw` / `raw_base64` are the ring's decoded copy re-encoded and `raw_sha256` proves the transfer but not what the page originally stored. Rows written by the version that closed each shape, and later, are exact. The 1.19.10 class is the one to be most careful with, because its page carries no corruption signal to warn you that the digest is a transfer proof rather than a preservation proof.
...
| `retired_props` | object | **Present only when declared.** A map of `prop name` => the v2 surface that replaced it, plus a `_note` describing the block for a human reading the schema. Drives the `retired_prop` refusal (#1007). Today eight components declare one, nineteen keys in total: `hero` (4), `section` (4), `cta` (4), `stats` (2), `testimonials` (2), `faq` (1), `embed` (1) and `logos` (1) |
...
| `roles` | array | **Present only when the component declares roles** — every v2 component, plus the `nav` and `footer` chrome. One object per role: `role`, `selector`, `groups`, `description`, and `obligations` when the role declares any (#1087), in declaration order. A v2 component has NO style slots, so without this the report would read as "nothing to style" about a component with a full design surface |
...
| `roles[].obligations` | array | **Present only when the role declares any** (#1087). One object per obligation: `kind` (`outranked_by_default` or `reached_only_by_inheritance`), `with` (the sibling role you must write alongside this one), and `why` (bounded prose, one line). These are the pairings no other channel reports: a value on one role that another role's default outranks, or that reaches a descendant only by inheritance and loses to the stylesheet's direct rule. The write envelope does NOT warn about the first kind — there is no finding for it — so this report is where a CLI agent learns it |
| `roles[].overlay_defaults` | object | **Present only when the role declares a non-empty one** (#1010, reported since #1144). The values that replace the role's defaults on a band the engine marks `data-pp-band-overlay` (an image under a scrim): a map of group → parameter → value, printed as declared. The shipped use is the accent inks (`{"typography": {"color": "@color-accent-on-overlay"}}`) |
| `roles[].within` | array | **Present only when the role declares it and at least one name survives the filter below** (#1010 review, reported since #1144). The roles whose elements enclose this one, which `udc_overlay_accent_off_scrim` reads for a light surface. Filtered to this component's role names that match the role-name pattern, so the report never prints a name that is not a role |
| `roles[].text_content` | bool | **Present (as `true`) only when the role declares it** (#1125, reported since #1144). The role's own element renders author text, so the own-surface ink finding (#1125) can name it |

Each entry carries **the schema's own keys and values, verbatim** — nothing injected, nothing dropped, order preserved — plus the map key promoted to `name` (props, recipes) or `slot` (style slots). (Role entries are a projection, not a copy: see the `roles[]` rows above; `within` is filtered to the component's role names and left out when none remains.) So a prop object is exactly its `schema.json` declaration with `name` added:
