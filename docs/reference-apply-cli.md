...
**If you key on error codes, this is the migration:** the fifteen keys each component declares in its schema's `retired_props` block — hero's `button_variant`, `button2_variant`, `spacing`, `width`; testimonials' `theme`, `title_align`; section's `theme`, `title_align`, `background_image`, `panel_cta_variant` (#1023); cta's `theme`, `background_image`, `button_variant`, `button2_variant` (#1026); and faq's `theme` (#1046) — moved from `unknown_prop` to `retired_prop`. Every other undeclared key still returns `unknown_prop`. The distinction exists so a caller can tell "you typo'd" from "this moved, and here is where" without string-matching prose, exactly as `retired_option` does for the chrome site options.
...
| `retired_props` | object | **Present only when declared.** A map of `prop name` => the v2 surface that replaced it, plus a `_note` describing the block for a human reading the schema. Drives the `retired_prop` refusal (#1007). Today `hero` (4 keys), `section` (4), `cta` (4) and `testimonials` (2) |
