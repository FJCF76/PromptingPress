...
**A ring slot holds one of two things (#818).** Normally a **composition snapshot** — the prior components, replayable by `restore_composition`. But when the bytes a write replaces did not decode to a composition (a `decode_error` page, or either sub-case of `unexpected_shape` — a valid-JSON scalar, or a valid-JSON object), the slot holds those **preserved bytes** instead. Before 1.16.13 such a write pushed *nothing*, so repairing a corrupt page destroyed the only recoverable copy of what was there — and the documented repair is exactly one full `update_composition` write, which made the recovery path the destructive one. The bytes are now kept, readable through `wp pp operate composition-history`, and refused by `restore_composition` (they are not a composition, so there is nothing to replay). The trade, stated: a preserved-bytes slot consumes a ring slot like any other, so a page corrupted and repaired repeatedly evicts good snapshots faster.
...
One row class is an exception to "the true bytes", and it is the one written before 1.17.1: an object-shaped prior filed the old way (#841) is reclassified on read, so its `raw` / `raw_base64` are the ring's decoded copy re-encoded and `raw_sha256` proves the transfer but not what the page originally stored. Rows written by 1.17.1 and later are exact for every class.
