...
| `udc_value_cannot_take_effect` | always for site chrome; a page's bands when `--post_id` is given. Reports only when a stored `udc` value is not painted | no (`severity: warning`) |
