...
"Did not decode to a composition" covers three shapes: bytes that are not JSON at all, valid JSON that is a scalar (`null`, `5`, `"text"`), and — since 1.17.1 (#841) — valid JSON that is an **object** rather than a list (a genuinely string- or gap-keyed one; `{"0":…,"1":…}` decodes to a real list and is unaffected). The object case used to be filed as a replayable snapshot, so the listing called it `restorable: true` and selecting it crashed the command; it is now preserved and refused like the others. Rings written before 1.17.1 are reclassified when they are read, so an old mis-filed slot refuses instead of crashing — with one caveat worth knowing before you copy anything out of it: for those older slots the `raw` / `raw_base64` bytes are the ring's own decoded copy re-encoded, not the exact bytes the page stored, because the exact bytes were never kept for that shape. Slots written by 1.17.1 and later are exact for every shape.
