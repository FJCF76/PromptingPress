...
That ring slot is not a composition snapshot — it holds the bytes a repair write replaced on a corrupt page (undecodable, a valid-JSON scalar, or a valid-JSON object), kept so the repair could not destroy the only copy of them (#818, #841). Nothing is wrong and nothing was lost. Read the bytes with `wp pp operate composition-history --post_id=<id>` (`raw_base64` is the exact copy, `raw_sha256` verifies it — exact for every slot written by 1.17.1 and later; a slot an older release mis-filed as an object-shaped snapshot is reclassified on read and hands back that object re-encoded, not the page's own bytes), then pass a larger `steps_back` — or the `history_index` of a row showing `restorable: true` — to roll the page back to a real composition.
