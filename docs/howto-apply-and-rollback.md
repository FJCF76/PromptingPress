...
**A restore can also be refused because the ring MOVED under you (#829), and that is the other good outcome.** Your selector is resolved against the ring you listed and then confirmed against the authoritative ring inside the write lock. If another writer — a CLI action, the dashboard editor, the AI chat — recorded a state on the page between your listing and your `execute`, the entry your selector names is no longer the entry you picked, so the restore is refused with `history_target_shifted` and **nothing is written**. Re-list with `wp pp operate composition-history` and select again. Worth knowing which selector you used: `steps_back` is *relative*, counting backwards from the newest entry, so every concurrent write changes what it means; a `history_index` taken from a current listing keeps naming the same entry until the ring evicts. On a busy page, list and then restore by `history_index`.

"Did not decode to a composition" covers four shapes: bytes that are not JSON at all, valid JSON that is a scalar (`null`, `5`, `"text"`), — since 1.17.1 (#841) — valid JSON that is an **object** rather than a list (a genuinely string- or gap-keyed one; `{"0":…,"1":…}` decodes to a real list and is unaffected), and — since 1.19.10 (#842) — a valid JSON **list whose entries are not components** (`["a","b"]`, or a list holding `{"component":"x","props":"str"}` where `props` should be an object). Both of the last two used to be filed as replayable snapshots, so the listing called them `restorable: true` and selecting one crashed the command; they are now preserved and refused like the others. Rings written before those versions are reclassified when they are read, so an old mis-filed slot refuses instead of crashing — with one caveat worth knowing before you copy anything out of it: for those older slots the `raw` / `raw_base64` bytes are the ring's own decoded copy re-encoded, not the exact bytes the page stored, because the exact bytes were never kept for those shapes. Slots written by the version that closed each shape and later are exact.

The list-of-non-components case has a wrinkle the other three do not: **the page itself is not corrupt.** `wp pp check page` reports it healthy, because the classifier only judges the container. So this is the one unrestorable slot you can meet without anything having gone visibly wrong with the page — the refusal is about whether that *slot* can be replayed, not about the page's health. Everything else is the same: the bytes are there, `raw_base64` recovers them, and an earlier entry rolls the page back.
...
That ring slot is not a composition snapshot — it holds the bytes an earlier write replaced, kept so that write could not destroy the only copy of them (#818, #841, #842). Four shapes land here: undecodable bytes, a valid-JSON scalar, a valid-JSON object, and — since 1.19.10 — a valid-JSON list whose entries are not components. Nothing is wrong and nothing was lost. Read the bytes with `wp pp operate composition-history --post_id=<id>` (`raw_base64` is the exact copy, `raw_sha256` verifies it — exact for every slot the current release writes; a slot an older release mis-filed as a replayable snapshot is reclassified on read and hands back its payload re-encoded, not the page's own bytes), then pass a larger `steps_back` — or the `history_index` of a row showing `restorable: true` — to roll the page back to a real composition.

**Does this mean my page is corrupt? Not necessarily, and since 1.19.10 quite possibly not.** The first three shapes only arise on a page that was corrupt and got repaired. The fourth — the list whose entries are not components — sits on a page `wp pp check page` reports as **healthy**, because the classifier judges the container and that container is a perfectly good list. So if the page reads clean, believe it: there is nothing to repair, and the refusal is telling you only that this one slot cannot be replayed. Step past it.
