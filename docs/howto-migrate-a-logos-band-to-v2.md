...
Nothing is *broken*. A stored `--logos-*` slot or a stored `theme` is simply not read any
more: the band renders as though neither were set, and no read surface says so (#1050). A
band that stored `theme: "inverted"` renders **light**.
