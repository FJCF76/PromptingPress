...
6. **Pseudo-elements remain deferred** — carried forward from ruling A3 unchanged, and recorded here so the two deferrals sit side by side. The live instance is `section.body_marker` / `panel_items_marker`, whose glyph is painted by `.pp-marker-list--{variant} > li::before`. In T4 the marker enums stayed structural props (the glyph is content) and the two colour slots retired with the narrowing stated in `retired_props`. Measured live exposure at the ruling: **zero**. Since #1028 their COLOUR has a home without a pseudo-element ruling: the `marker.color` group param on the holding role sets `--pp-list-marker-color` on the list's own box, which the glyph inherits. The glyph itself stays deferred, recorded rather than implied.

7. **No `_css` at item grain** — *added at implementation time, 2026-09-21, orchestrator ruling during #1101; overridable, and flagged in the sprint report the way the A3/T2 sub-ruling was.* B1 grants an item map "the same grammar", and the ratified six exclusions say nothing about the Layer-2 `_css` valve, so items would have inherited raw-CSS authoring by silence. They do not. The reason is the T8 lesson taken in advance: `_css` is the one key whose namespace belongs to the author rather than the registry, and every guard that makes it safe at band grain — the drop ledger, `udc_css_overrides_group_value`, `udc_css_unchecked_property`, the `PP_UDC_MAX_EMIT_DROPS` accounting — reports with a BAND locator and has no item-located channel yet. Raw CSS at a brand-new tier whose disclosures cannot name the card it came from is the accepted-stored-unreportable shape this contract exists to refuse. An item map naming `_css` is refused, not ignored. Lifting this is its own ruling, and it needs the item locator threaded through the drop ledger first.

**A note on what B does NOT do, so it is not re-derived next sprint.** The plan review's outside voice proposed a cheaper alternative: a per-item ENUM (`items[].tone: "inverted"`) plus one extra declared role, which would meet the measured need — 4 pages, 1 card each, 4 colour parameters — with zero engine change, and would travel with the item on reorder exactly as exclusion 3 demands. It was considered and **refused**, for a reason that is a standing project rule rather than a preference: *a dogfood reveals a missing GENERIC CAPABILITY; it never licenses encoding the benchmark's own domain model — severity enums, named variants — into a schema.* A `tone: "inverted"` enum is precisely that encoding. It would also answer only the case already measured, while B5's declaration serves testimonials, faq, logos, stats, table and `section.panel_items` with no further engine work. The generality is the ruled choice; the enum is the shape it was chosen over.
