...
- **I8 (#909 CRITICAL): every mutating step is CAS-covered by a baseline the conversation EARNED for the page it targets.** The only invariant whose violation converts a refusal into an acceptance. PINNED in the Sprint-0 truth-spine tests (the fix stays deferred per §7; the pin records the truth).
