...
| id | pins | shape |
|---|---|---|
| T-1 | §3.1 base ownership | the PP-owned table equals **derive(core `post` on the pinned WordPress version) − §4 + Δ**, and a stored snapshot of core `post` equals live core `post` (the clause that fails on drift; never the table compared to itself). Previously worded: the PP-owned table equals core's `post` context on the pinned WordPress version, and diverges loudly on drift |
