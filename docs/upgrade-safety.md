# Upgrade Safety: Theme Integrity & Update Blocking

PromptingPress ships a build manifest of file hashes and checks the live theme
against it. If your theme files have drifted from what shipped, an update is
**blocked before it can overwrite or delete your changes**. A daily background
check keeps the warning current, and a single filter lets you override the block
when you mean to.

This document has three parts:

- [Reference](#reference) — the exact commands, hooks, options, and exit codes.
- [How-to: an update was blocked, what now?](#how-to-an-update-was-blocked-what-now) — the remediation workflow.
- [Explanation: why updates are blocked on local drift](#explanation-why-updates-are-blocked-on-local-drift) — the design rationale and trade-offs.

> The short version for site builders: customize sites through **design tokens,
> fonts, and compositions** (stored in the database, they survive updates). Don't
> hand-edit files under `templates/`, `components/`, or `assets/` — those are
> release artifacts, and a theme update replaces the whole directory.
...
Both blocking states honor a single override filter. Add a filter that returns
`true` to let the update proceed anyway:
...
drift without waiting for an activation, update, or manual CLI run. Switching away
from the theme clears the event and the stored options.

Note: WP-Cron is traffic-driven. On a low-traffic site or one with `DISABLE_WP_CRON`,
the daily refresh may lag — but the update block always runs a **fresh** check at
update time regardless, so the safety guarantee does not depend on cron firing.
...
- **An update was blocked** — shown when `pp_last_blocked_update` is set (a record
  written each time the guard blocks an update, including silent auto-updates, with
  the timestamp and affected-file counts). This record clears automatically once an
  integrity check returns `safe`.
...
## How-to: an update was blocked, what now?

You tried to update PromptingPress and saw an error like *"PromptingPress update
blocked: local theme files have changed (2 modified, 1 extra)."* That means the
update would have overwritten or deleted changes made directly to theme files. Here
is how to get unblocked safely.

### Prerequisites

- WP-CLI access to the site (`wp` on the server), or the ability to add a small
  site/must-use plugin.

### Step 1 — See exactly which files drifted
...
theme root. This tells you what is about to be lost.

### Step 2 — Decide what to do with each change

Pick the path that matches your situation:

**A. The changes were accidental, or you don't need them.**
...
**B. The changes are real site customizations you want to keep.**
Move them off the theme files and into update-safe storage, because the next update
will replace the theme directory no matter what:

- **Colors, fonts, spacing, radius** → set design tokens via the apply layer, which
  stores overrides in the `pp_token_overrides` database option (survives updates).
  Use the `update_design_token` apply for tokens and `enqueue_font` for fonts. From
  PHP:
...
- **A genuinely structural change** that tokens and compositions can't express (a new
  template, a changed component) is a **product/release change**, not a site
  customization. That belongs in a theme release, not a hand-edit. Open an issue
  rather than editing the file in place.

Once the change lives in tokens/compositions/content, restore the theme file
(path A) and update.

**C. You understand the risk and want to update anyway.**
...
After updating, confirm the install is clean and the warning is gone:
...
- **"integrity cannot be verified" (`invalid_manifest`)** — the shipped manifest is
  ...
  cannot be ruled out; use the override filter (path C) if you must update.
- **An auto-update silently failed** — check the admin notice or
  `wp option get pp_last_blocked_update`. A blocked background auto-update never shows
  a live error, so this stored record is where the reason lives.
- **`extra` files you didn't create** — editor backups (`*.css~`), `Thumbs.db`, or a
  ...
  deletes them. Remove the stray file, or override with path C.

---

## Explanation: why updates are blocked on local drift

### The problem

A WordPress theme update does not patch files in place — it **replaces the entire
theme directory**. Anything you hand-edited under `templates/`, `components/`, or
`assets/` is overwritten, and any file you added there is deleted. Before v0.11.0,
PromptingPress could *detect* that files had drifted and show a warning, but nothing
stopped an update from running. The warning told you about the damage after it was
done, which on a background auto-update means no human saw it at all.
...
customize a specific site. Site customization flows through mechanisms that live in
the database and survive updates — design tokens (`pp_token_overrides`), enqueued
...
discoverable and consistent: a corrupt manifest fails closed (blocked) like real
drift, but the same documented override frees you, so a bad manifest can never
permanently strand a site on an old version.

### Trade-offs

- **Blocking on `extra` files can false-positive.** A stray `components.css~` backup
  or a plugin that writes into the theme directory will block an update. We accept
  this because a theme update genuinely *deletes* those files — the block is correct
  even when the file is junk. The override filter is the relief valve.
- **`invalid_manifest` fails closed.** When integrity can't be verified, the safer
  choice is to block rather than risk silently overwriting real work. The cost is
  that a packaging bug in the manifest would block updates until the manifest is
  restored — mitigated by the override filter.
- **Cron freshness isn't guaranteed.** The passive admin warning can lag on
  low-traffic or `DISABLE_WP_CRON` sites. The block doesn't depend on it: it always
  re-checks at update time.
- **The guard is theme-level.** It protects the active PromptingPress theme on a
  single site. It does not cover paths where the theme code isn't loaded (an inactive
  theme, some multisite network-admin flows, or an upload-overwrite of a theme that
  isn't active). Those are out of scope for v0.11.0.

### Out of scope (deliberately, for v0.11.0)

Child-theme or runtime override support, ZIP backup of drifted files before update,
and multisite-aware blocking are not included. The "STOP and escalate" instruction
for structural changes is the v1 answer; a supported override layer is a tracked
follow-up.
