...
The read-back also cannot substitute its own idea of an array for a stored one. `wouldLoseArrayData` makes the sync skip a field whenever the DOM read could not have come from editing: no rows read at all (either the stored value is not an array so the container rendered nothing, or the read came back empty while rows were stored, which is a failed read either way), every row read as `{}` (no sub-controls exist, the permanent state of a pass-through array), or a row of empty strings sitting over an original item that is not an object (a stored scalar renders controls that read back `''`). A row of empty strings over an OBJECT original is the user clearing fields and writes through, and so is emptying an array by removing its last row, which rewrites the buffer directly rather than going through the read. The accordion therefore cannot repair a value it could not display; the JSON view is the way to fix one. `tests/js/pp-editor-form-sync.test.js` pins all of it.
