...
- No writing to WordPress Custom CSS (Appearance > Additional CSS) for theme styling. Styling goes through design tokens, a v2 component's role `defaults` in its `schema.json`, or `components.css` for a component not yet rebuilt.
...
- **A v2 component's designable values do not go in `assets/css/` at all.** A component
  ...
  classified. As of #1066 that covers testimonials, hero, section, cta, faq, table, embed, nav and footer.
