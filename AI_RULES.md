...
- No writing to WordPress Custom CSS (Appearance > Additional CSS) for theme styling. Styling goes through design tokens, a band's `udc` map (its roles' groups and parameters, and `"_css"` for a property no group owns), or `components.css` for a component not yet rebuilt. A v2 component's role `defaults` in its `schema.json` are the shipped baseline, not the place to express one site's design — they are repo-owned and a release overwrites them.
...
The logic under test is in `assets/js/pp-editor-logic.js`. When editing `escapeHtml`, `getJsonContextFromText`, `validateCompositionData`, `getInsertPosition`, `buildAccordionData`, `serializeAccordionData`, `deepDiff`, `checkSerializationInvariant`, `formatDiffsForIssue`, or the row-guard family (`wouldLoseArrayData`, `reconcileArrayItems`, `reconcileSubFieldTypes`, `subFieldIsDisplayOnly`, `subFieldIsTypedScalar`, `nonContainerValueDiffs`), run tests before committing.
...
Two more rules hold on the same path. Values reach the markup through `esc(value)` and are never pre-coerced with `String(value || '')`: that idiom is falsy-based, so it blanks a `0` or `false` the author meant, and the next read-back takes the blank as the value. Note which values that is, because JavaScript and PHP disagree: of the values `JSON.parse` can produce, only `0`, `false`, `null` and `''` are falsy (JavaScript also has falsy `undefined` and `NaN`, but no JSON document decodes to either), and the string `'0'` is NOT — so a `'0'` fixture cannot exercise the idiom, and a test that uses one is a round-trip regression rather than a probe of this rule. Since #745 a `0` or `false` under a prop the schema DECLARES `type: "string"` never reaches that markup at all: it is reported as drift and the composition is routed to JSON-only mode before anything renders, because a text control would hand it back as its own text and the write path would then accept the string. What still arrives falsy is a `0`/`false` on an undeclared pass-through prop, or on an items[] sub-key declaring `number` — the branch this rule now protects. Since #805 the `array` and `object` sub-keys are no longer among them: they render through a READ-ONLY display built from `JSON.stringify`, never through this branch, because a text control cannot carry a list or a map at all. A `number` sub-key still renders here, and must still render a stored `0` as `0`; what it must not do is come back as the string `"0"`, which `reconcileSubFieldTypes` settles on the READ by comparing it against the text it was rendered from. It stays stated as a rule about the renderer rather than about which values can reach it. And every reader that acts on the JSON buffer from a user gesture calls `flushPendingFieldEdits()` first — the six structural row handlers, save, publish, and the accordion-to-JSON leg of the view toggle — because the sync is debounced and an edit made inside that window is otherwise still in the DOM, unwritten, when the buffer is read. The rule is "reads the buffer", not "re-renders from it": save and publish do not re-render, they POST, and posting the pre-edit value is the same loss. Flushing moves buffer content only; the `expected_version` baseline the save carries (#13) is untouched. Two readers are deliberately exempt: `runValidation` and `runPreview` are themselves debounced and the sync re-drives both on its trailing edge, so they settle without anyone flushing for them. The JSON-to-accordion leg is exempt for the opposite reason — there the buffer is what the author has been hand-editing, so a flush would write a read of the hidden form over it.

The read-back also cannot substitute its own idea of an array for a stored one, and it answers that at three grains. `wouldLoseArrayData` skips the whole field when the read says nothing about any row: no rows read at all (the stored value is not an array, so the container rendered nothing), every row read as `{}` (no sub-controls exist, the permanent state of a pass-through array), or fewer rows read than were stored. `reconcileArrayItems` then settles the surviving rows one at a time, because a read can be right about one row and wrong about its neighbour, and refusing the field to protect the bad one would discard the edit the author made to the good one.

The per-row rule is the same question asked smaller: could the author have produced this read by editing? Content in the read always wins, so the merge never fabricates a value — it only declines to replace one. An all-empty row is an edit only if every key the stored item held had a control to be cleared in. So `{title:'a'}` read as `{title:''}` is the author clearing it and lands, while `{foo:'bar'}` read as `{title:'',body:''}` keeps its stored value: the accordion renders a control per DECLARED sub-key, so `foo` was never on screen and reads back absent. That is why the test is the stored item's own KEYS, not its type — an array row can be an ordinary object and still carry keys the schema does not declare. Each restored row is warned about by index. Since #643 that preserved key also decides whether the next save works, and the two branches diverge. Leave the row alone and the key survives the sync, so the save is REJECTED with `unknown_prop` naming it. Type into any declared control in that row and the DOM read wins, so the undeclared sub-key is dropped from the buffer — the accordion only reads the sub-keys `field.items` declares — so that specific `unknown_prop` rejection no longer fires, and nothing warns that a value was discarded (the save can still fail for any other reason, such as a missing required item field). So the console warning is the operator's cue to decide deliberately in the JSON view, because editing the row through the accordion is itself a silent repair. Emptying an array by removing its last row is unaffected: that rewrites the buffer directly rather than going through the read. Since #805 a THIRD rule runs between the whole-field veto and this per-row merge, at the grain of one SUB-KEY: a sub-key whose declared type the row controls cannot round-trip keeps what was stored, because no control ever showed it to be edited. Order is load-bearing in both directions — after the whole-field veto so that veto still judges the raw read, and before the per-row merge because a read-only sub-key is ABSENT from the read, so running it second would make the per-row test ("was every key the stored row had on screen to be cleared?") restore the whole row and silently discard a legitimate clearing. It never fabricates: a sub-key the stored row does not have is not restored. It does not warn, deliberately — unlike the two guards either side it describes the designed steady state of every page holding a list, it would fire on every keystroke of a debounced sync, and the reason is already on screen beside the control. The accordion still cannot repair a value it could not display; the JSON view is the way to fix one. `tests/js/pp-editor-form-sync.test.js` pins all of it.
