...
## [v1.12.18] — 2026-08-09 — the `aliases` schema field is retired, and a prop now accepts exactly the values it advertises (#606)

**Three sibling issues emptied every alias surface in the theme (#603 slot names, #604 prop keys, #605 the `theme: "dark"` value). This one disposes of the machinery they left behind. The `aliases` schema field, its validator, the arm of the strict-enum write gate that consumed it and the AI-catalog line that disclosed it are all deleted. `aliases` is now an UNKNOWN definition key: a schema that declares one fails CI, on a prop, on a slot, and on a nested `items[]` field. Nothing rendered or written changes, because no shipped prop had declared one since #605.**
