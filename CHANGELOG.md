...
The through-line extends v1.13.0's rule one layer deeper: a write that reports success must produce the page it says it produced, and a diagnostic that reports a problem must describe the problem that exists. Before this gate a nested field could store anything and render something else, a repair loop could chase the wrong band with the wrong slot list, a "fixable" rejection wore an "impossible" label, and five separate documentation surfaces advertised a visual that provably cannot paint. Each of those is now either true or rejected by name.

**What breaks, stated plainly.** Compositions that store non-scalar or out-of-set values inside `items[]` entries were always rendering something other than what they stored; the three whole-composition actions now reject such a page by name — including when the edit targets a different band — and `wp pp check page` / `wp pp validate site` exit non-zero on it. That is the strict gate working as designed. There is no migration, no coercion, and no warning-only tolerance; the recovery path is rewriting the offending entry with declared values. Freshly authored content is unaffected.

No code changes in this release beyond the version bump — see the [v1.13.1]…[v1.13.15] entries below for the substance.
...
The lookup went to a comparison rather than to escaping. `CSS.escape` targets identifier context, not the quoted attribute-value position these selectors use, so taking that route would have meant reasoning about quoting discipline at each of three call sites and depending on the function being available; comparing the attribute value that the DOM already holds needs neither. The three sync lookups share one helper, `findByCompField`, and each is scoped to its own component card — the card index is a loop counter rather than a composition key, and it is the one value in the function that can safely be interpolated. Two selector concatenations remain, in the move-up and move-down handlers; both interpolate a `parseInt`-derived, bounds-checked index, so the value reaching the selector is always a number, and each now says so. The accordion's expand/collapse lookup moved from `$('#' + id)` to `document.getElementById`, which takes a raw id and parses nothing.

A lookup that resolves nothing is now reported and skipped rather than written back. Two paths could reach the serializer with a value that came from the DOM failing to answer rather than from an edit: an array whose container did not resolve arrived as zero rows, which is the same value as an array the author emptied, and a scalar whose control did not resolve was skipped in silence. The array case now stops at the container and leaves the field as it was, the scalar case reports the same way the array branch already did, and the existing array guard is untouched and still covers rows that resolve individually.

### Fixed

- **Field lookup during form sync (`assets/js/pp-admin-editor.js`).** `findByCompField($scope, compIdx, fieldName, baseSelector)` selects by static structure, then filters on exact `data-comp` / `data-field` attribute values read with `getAttribute`. It replaces interpolation at all three sync call sites: the array container, the array sub-field control, and the scalar control. `compIdx` is stringified for the comparison; field names need no conversion, since every caller sources them from `Object.keys()`.
- **Per-component scoping.** Each component's lookups are scoped to its own `.pp-accordion-card`. The predicate already required a matching `data-comp`, and a card holds exactly the controls carrying its index, so the match set is unchanged while a scalar lookup no longer rescans every control in the accordion.
- **Value reads resolve form controls only.** The two lookups that end in `.val()` select `input, textarea, select`, so the array container and its add/remove buttons — which also carry `data-comp` and `data-field` — can never be read for a value.
- **Unresolved lookups no longer reach the serializer.** The array branch returns at an unresolved container; the scalar branch reports an unresolved control instead of skipping silently.
- **Expand/collapse panel lookup.** `$(document.getElementById($header.attr('aria-controls')))` replaces `$('#' + ...)`.

### Docs

- `AI_RULES.md` records the read-path rule alongside the existing `escapeHtml` write-path rule: the sync path puts nothing dynamic into a selector.
- `README.md` lists field lookup in the JS unit-test coverage summary.

### Tests

- `tests/js/pp-editor-field-lookup.test.js` (new, 16 cases) boots the real `pp-admin-editor.js` under jsdom and drives real edits through the real 300ms-debounced sync, asserting on the JSON the editor actually serialized. Covers names containing quotes, brackets, backslashes and dots, array props and sub-keys with such names, per-component isolation, both unresolved-lookup paths, and ordinary names. Field names are built as JS constants and inserted with `JSON.stringify`, so a key is exactly the characters the constant holds. Source tripwires assert no `data-field` / `data-comp` selector is built by concatenation.
- `tests/js/pp-editor-dom.test.js` comments re-scoped: it pins the array sub-field naming contract, not the shipped lookup.

---

## [v1.13.0] — 2026-08-12 — the component authoring contract: what a schema advertises is what the page renders (rollup)

**v1.13.0 is the rollup release of the Component Authoring Contract gate — nineteen working versions (1.12.3–1.12.21) verified as one line.** Fifty-one style slots and eleven props moved to one canonical vocabulary across all ten composable components (#576), and the families that had shipped a single member — band heading rhythm, per-instance rings, logo sizing, card and avatar images — are now complete on every component that can use them (#584, #577). Every band gained a real text-measure control and a `--measure-heading` token, and two cross-component slot leaks were severed (#578). One hundred and sixty definitions now declare, machine-readably, the configuration they need before they do anything, with a non-blocking `inert_slot` advisory when an author writes one anyway (#580). Every enum prop rejects what it used to accept-and-ignore (#579), twenty-five schema defaults now state the value that actually renders (#581), the documented `pp_logo_alt` option finally reaches both chrome logos (#582), and forty ratified literals carry a stated reason and a reopening condition where an authoring agent will read them (#585). The four alias surfaces that made a name mean two different things are gone: the 51-entry style-slot map (#603), the 13-entry prop-key map with the `variant` read migration and the `type` key alias (#604), the `theme: "dark"` input value (#605, the emitted `--dark` class stays), and the `aliases` schema field and its machinery (#606) — plus the unbounded edit-distance slot repair in the chat preview path (#607). `wp pp check page` and `wp pp validate site` now read the shared findings engine, so the breakage those removals intend is visible on the surfaces an operator queries first (#622). Table, embed and logos got their first component-level regression net before any of it landed (#583), a spec-sheet row stacks on phones instead of squeezing its value into a 170px sliver (#568), and the accordion editor's markup escaping was unified while the chat's error reflection gained bounds.

The through-line is a single rule: a write that reports success must produce the page it says it produced. Before this gate a slot could be declared and never routed, a prop could advertise three values and accept a fourth, a default could describe something that had not rendered for releases, and four separate mechanisms rewrote stored names behind the author's back — on read, at render, or at write — each emitting nothing to say it had. Every one of those is now either true or rejected by name. The removals are validation STRENGTHENING, not relaxation: the retired names return to the strict unknown-prop and `invalid_style_slot` gates rather than being quietly healed. Two deliberate render changes carry the gate: the hero headline is no longer capped at 12 characters (#578), and ten previously dead slots now paint (#577), each named and measured in its own entry. `restore_composition` still restores verbatim and reports rather than blocks (#233) throughout.

**What breaks, stated plainly.** Compositions written before the vocabulary freeze carry retired slot names, retired prop keys, and `theme: "dark"`. Those declarations drop at render — the page still renders, `theme: "dark"` renders as the default band — and the three whole-composition actions (`create_page`, `update_composition`, `update_component`) reject such a page by name, including when the edit targets a different band. `wp pp validate site` exits non-zero on it, which is the #622 feature working as designed rather than a regression. There is deliberately no migration, no coercion and no warning-only tolerance; the recovery path is rewriting the band with canonical names. Freshly authored content is unaffected and pinned to stay that way, so a fresh install is clean.

No code changes in this release beyond the version bump — see the [v1.12.3]…[v1.12.21] entries below for the substance.

---

## [v1.12.21] — 2026-08-12 — the intended #604 breakage is now visible to the surfaces you query first (#622)

**#604 removed the legacy prop-key alias map, the `variant` read migration and the `type` -> `component` alias, and stale-data breakage was the stated outcome. But the two read-only diagnostics an operator or agent queries FIRST did not report it: `wp pp check page` and `wp pp validate site` ran only the styling and smell checks, never `pp_validate_composition_errors()`, so a page whose stored composition current write rules REJECT printed "no composition smells" and "Site validation passed" right up until its next edit was refused. Both commands now read the shared findings engine `restore_composition` already uses, so error-severity findings reach them for free. Error-derived findings carry the composition offset of the band that owns them instead of a hardcoded `null`. A missing-required-prop error names the undeclared prop keys the item is actually carrying. And the chat's undo card styles each restore finding from its own `severity` instead of rendering every one as a warning.**

The reach fix is one function, not a third validator: `_pp_cli_page_diagnostics()` (`lib/cli.php`) calls `_pp_composition_findings()` and splits it on `severity`, so both commands render errors and advisories through one code path and every rule added to either shared engine from now on reaches both for free. `wp pp validate site` — the comment above it calls it "the command CI runs" — now exits non-zero for an error-severity finding. On content written before the vocabulary freeze that is a deliberate red: the page really would refuse its next edit, and reporting it clean was the bug. `wp pp check page` reports the same errors and keeps its exit contract; it is the per-page inspector, not the gate. The shipped starter composition and freshly authored content are clean and pinned to stay that way, so a fresh install is unaffected.

The locator rides as `WP_Error` data rather than as a message prefix. Those messages are the write path's public rejection strings, returned verbatim to `create_page`, `update_composition`, `update_component` and the editor save; rewording all of them is a wider change than this one. Data is inert to every existing consumer — nothing in `lib/` read `get_error_data()` before this — and `_pp_composition_findings()` reads it to fill the `index` that was always `null`. `duplicate_component_id` keeps no locator on purpose: it is the one cross-item rule, it belongs to no single band, and it already names every colliding index in its message.

The prop-key help is derived from the component's current schema and nothing else. Post-#603/#604/#605/#606 there is no alias map to consult, and a static "formerly known as" list would be that machinery again under another name. Naming the keys that ARE present and unrecognized is the honest form: when a value sits under a retired name, that name appears in the message and the operator can see what to rename. The list is bounded (10 keys, 64 characters each, with a tail reporting the true total) and stripped of control and format characters, matching the reflection posture #633 set for the sibling style-slot path.

### Fixed

- **`wp pp check page` and `wp pp validate site` reported a stale page as clean (#622).** Both now read `_pp_cli_page_diagnostics()` (`lib/cli.php`), which splits `_pp_composition_findings()` by severity and adds `pp_validate_composition_styling()`. Errors print first, tagged as rejected-on-write. `_pp_cli_page_fails_site_validation()` is the extracted, unit-testable exit-code decision shared by both commands, so neither can gain a diagnostic bucket the other silently skips.
- **Error-derived findings carried no band locator.** `_pp_composition_findings()` (`lib/actions.php`) hardcoded `'index' => null` for every error. `pp_validate_composition_errors()` (`lib/admin.php`) now builds each per-item error through `_pp_composition_item_error()`, which stamps the composition offset as `WP_Error` data; `pp_composition_error_index()` reads it back. On a page with two `cta` bands the operator can finally tell which one is dead. Errors and smells share one rendered format, `[type] index N: message`.
- **A missing-required-prop error never named the retired synonym holding the value.** The required-prop check fires before the unknown-prop gate and ends the item, so the key next door was never mentioned. The message now appends the item's undeclared prop keys and the component's declared props. Schema-derived; no alias lookup exists and none was added.
- **The chat rendered error-severity restore findings as warnings.** `ppChatAppendUndoFindings()` (`assets/js/pp-ai-chat.js`) passed one fixed class for every item. `ppChatFindingClass()` now selects per item from `severity`, and the overflow disclosure names the hidden items from their own severities. The card's heading stays a warning that says "Restored": the restore succeeded and is still never blocked (#233).
- **The error engine could kill the commands it was just wired into.** Reached through the CLI it runs over compositions that never passed write validation, where three shapes were fatal or corrupting: a non-array `props` bag threw a `TypeError` from `array_key_exists()`; a non-scalar style-slot value emitted an "Array to string conversion" warning (into the AJAX findings body under `WP_DEBUG_DISPLAY`) or an uncaught `Error` for an object; and a non-list composition made the new `int` locator parameter a `TypeError`. All three now report a finding instead, honouring the #144 report-don't-die contract these surfaces exist for.
- **Stored bytes reached the terminal unescaped.** `_pp_cli_finding_line()` strips Unicode control and format characters from both the type and the message, with a byte-wise fallback that keeps the readable text when the payload is not valid UTF-8 rather than discarding the whole diagnostic.

### Docs

- `AI_CONTEXT.md`, `ai-instructions/validate-site.md`, `ai-instructions/composition.md`, `ai-instructions/add-component.md`, `docs/reference-apply-cli.md`, `docs/howto-apply-and-rollback.md` and `docs/AI_IMPLEMENTATION_RECIPES.md` all recorded that only `restore_composition` surfaced a stored `unknown_prop`, that error findings carry no `index`, or that `wp pp check page` fails on template-owned chrome. Every one is corrected, including the worked `findings` example that still showed `"index": null` for a per-item rule.
- `ai-instructions/validate-site.md` gains the error-severity class as a numbered check with its own row group in the what-to-do table, and states plainly that a site carrying pre-freeze names goes red here on purpose.
- `docs/AI_IMPLEMENTATION_RECIPES.md` records the mechanism protecting the new invariant: a rule added to `pp_validate_composition_errors()` must be built with `_pp_composition_item_error()`, or it silently reports `index: null` and breaks the contract that only `duplicate_component_id` lacks a locator.

### Tests

- `tests/DiagnosticReachTest.php` (33 cases) drives `PP_Check_Command::page()` through the WP_CLI stub against seeded post meta, and pins the locators, the cross-item null, the enriched message, the bounds on the reflected key list, the corrupt-row survival paths, and the equivalence of the CLI bundle with the shared findings engine — so a second validator cannot creep in unnoticed. Section 14.1: the enriched message is exercised through `pp_execute_action('update_composition')`, not only the validator.
- The shipped starter composition and a freshly authored one are pinned to produce zero error-severity findings, so the new exit code cannot red a fresh install.
- `tests/js/pp-ai-chat-undo-findings.test.js` grows from 7 to 12 cases covering per-item severity, mixed lists, overflow styling, the derived disclosure noun, and the unknown-severity degrade.
...
Hero style slots are emitted as inline custom properties on the hero root, so they inherit down to both CTA anchors. When the second CTA is authored as the filled `primary` variant it also matches the shared premium button cascade, which is where #514's new `--hero-button-*` slots are read — so the primary's fill and elevation leaked onto it. The second CTA's own `--hero-cta2-bg` had the opposite problem, and had it since long before #514: it was consumed only as `background-color`, and the premium gradient `background-image` painted straight over it. One rule fixes both. Re-declaring the three slots on the second CTA element beats the inherited value, and pointing `--hero-button-bg` at `--hero-cta2-bg` (rather than merely clearing it) routes the second CTA's own fill into the gradient-clearing chain: unset, the `var()` fails to substitute, the property becomes guaranteed-invalid, and the premium fallback chain resolves to today's gradient; set, the flat color collapses the `background` shorthand and clears the gradient. Border-follows-fill mirrors the idiom #514 gave the primary. Hover is unchanged: a flat per-instance fill still reverts to the premium gradient on hover, the shared trait both buttons carry.

### Fixed
- The second hero CTA no longer inherits `--hero-button-bg` / `--hero-button-color` / `--hero-button-shadow` from the hero root in any variant (assets/css/components.css). Flattening the primary with `--hero-button-shadow: none` no longer flattens the second CTA with it — that coupling was the leak; the second CTA has no elevation slot of its own today (#526).
- `--hero-cta2-bg` now paints a filled (`primary`) second CTA instead of sitting invisible under the premium gradient, routed through the same winner chain `--hero-button-bg` uses for the primary (#526).
- A filled second CTA's border now FOLLOWS `--hero-cta2-bg` when `--hero-cta2-border` and `--hero-accent` are unset, so a fill-only recolor keeps a matching ring instead of a `--color-accent` one (#526).

### Docs
- `components/hero/schema.json`: `--hero-cta2-bg` documents the gradient replacement and the hover revert; `--hero-cta2-border` documents border-follows-fill; `--hero-cta2-hover-bg` documents the hover gradient; `--hero-button-*` state they apply to the primary button only.
- `ai-instructions/style-component.md` no longer instructs the chat AI that a filled second CTA picks up `--hero-button-*` (it no longer does), and documents the second CTA's own fill recipe; `ai-instructions/retheme.md` extends the border-follows-fill idiom to the second CTA (#526).
- Corrected three stale comments in assets/css/components.css that claimed the `--hero-button-*` rules could never match the second CTA — that match was the leak.

### Tests
- `tests/e2e/style-render.spec.ts`: six computed-style pins at 1280 and 375 — the primary's slots do not reach a filled second CTA, `--hero-cta2-bg` clears the gradient and paints (border matching), and both CTAs stay byte-identical with no fill slots set. Four go red if the isolation rule is removed, two if border-follows-fill is. CSS-text pins cannot see this bug class: a background-color under a gradient is present in the text and invisible on screen (#526).
- `tests/StyleSlotContractTest.php`: pins the isolation rule's three declarations, order-independently, and its unqualified selector; the "no stylesheet rule declares a schema slot" guard gains an exact, documented exemption list and now diffs by COUNT, so a duplicate rule re-declaring the same slot can no longer slip through (#526).
- `tests/ActionsTest.php`: authoring-path coverage for the combination that triggers the leak (primary fill slots + a filled second CTA styled with its own), plus the reject branch for an invalid `--hero-cta2-bg`. `tests/HeroCompositionTest.php`: pins the markup contract the CSS keys on. `tests/ComponentPropsTest.php`: the per-variant slot count now counts fill consumptions specifically, so the new declaration cannot mask a deleted variant rule (#526).
...
Type enforcement is schema-driven: a prop declared `type: "string"` rejects a non-scalar, `type: "number"` rejects a non-numeric, `type: "array"` rejects a scalar, and an object-item array (one that declares `item_type: "object"`, like `grid.items`) rejects any entry that is not an object. There is no per-component code; a new prop is enforced the moment its schema declares a type, and the three existing bounded families (numeric `min`/`max`, strict enum, string-array bounds) keep their precise messages on top. The link-URL family works the same way through a `format: "link_url"` annotation on `cta.button_url`, `hero.cta_url` / `cta2_url`, `section.panel_cta_url`, and `grid.items[].link_url`: a site-relative path, an `#anchor` (dev's `#booking` links are fine), a protocol-relative URL, `mailto:`, `tel:`, and every other allowed protocol stay accepted, while a disallowed protocol (`javascript:`, `data:`, and their control-character-obfuscated variants) is rejected at write time with a message that names the fix. Both rules run in the shared validator, so a legacy composition that already holds a bad value is reported by `restore_composition` as a finding and never blocks undo.

### Fixed
- The shared composition validator (`pp_validate_composition_errors()`) now enforces each prop's declared schema `type` — string rejects non-scalars, number rejects non-numerics, array rejects scalars, and object-item arrays reject non-object entries — so a wrong-typed value is rejected at write time instead of rendering as `Array`/PHP warnings behind `ok:true`. Per-type unset sentinels (`null`, `""`, and `[]` for arrays) preserve each prop's default (#507).
- Link-destination props (`cta.button_url`, `hero.cta_url` / `cta2_url`, `section.panel_cta_url`, `grid.items[].link_url`) reject a value whose protocol `esc_url()` would neuter into a dead button, while keeping every value that renders as authored (site-relative, anchor, protocol-relative, `mailto:`, `tel:`, allowed schemes). Control characters embedded in a disallowed scheme are stripped before the check so an obfuscated `javascript:` cannot slip through as a relative path (#507).

### Docs
- `docs/reference-apply-cli.md` documents the two new write-time rules (schema-typed prop values and the `link_url` format family) alongside the existing unknown-prop / chrome / duplicate-id rules, including the exact rejection envelopes and the restore-reports-never-blocks behavior (#507).

### Tests
- `tests/SchemaValidationTest.php`: the per-type accept/reject matrix (non-scalar string, non-numeric number, scalar array, non-object and JSON-list entries), the mixed-entry `panel_items` exemption, the `link_url` helper accept/reject matrix (including leading-whitespace and control-character obfuscation), the fail-closed allowed-protocol set, and a registry-wide drift-catcher that runs `pp_validate_composition` against every `format: "link_url"` prop to prove wiring. `tests/ActionsTest.php`: Section 14.1 authoring-path proofs through `create_page` / `update_component` / `update_composition` for both accepts and rejects, plus a `restore_composition` finding-without-blocking proof. `tests/bootstrap.php`: a `wp_allowed_protocols()` stub mirroring the WordPress default set (#507).
