...
### Fixed
- **`pp_snapshot_token_overrides()` now fails closed on lock contention.** It read `pp_token_overrides` under the advisory token lock for an atomic baseline, but passed a plain `get_option()` read as the lock-failure fallback. When `GET_LOCK` timed out or errored, it silently returned that stale, non-atomic read, contradicting `lib/cli.php`'s own "read under the lock for an atomic baseline" claim. A `wp pp apply preflight` racing another writer could freeze a baseline that never existed atomically, and a later `apply restore` would silently revert to it. The snapshot now returns `null` on lock failure instead of degrading, and `wp pp apply preflight` treats a `null` snapshot as a hard failure: it records nothing (leaving both the action-execute and apply gates fail-closed) and errors so the operator can retry once contention clears. 3 new PHPUnit tests (in-lock read on success, `null` on lock-busy, `null` on GET_LOCK error). Found by adversarial review during #98. (#200)
