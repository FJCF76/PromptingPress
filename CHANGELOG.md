...
### Fixed
- **Color values reject `transparent`, `currentColor`, and token references (#230).** `_pp_validate_color()` (`lib/apply.php`) accepts the two CSS-wide keywords (case-insensitive) and a single bare `var(--token)` reference parsed by the new shared `_pp_parse_token_reference()` helper (`\z`-anchored so a trailing newline cannot slip through) — the referenced token must be registered and color-typed. Product defaults that ship as `var()` (`--text-meta-color`, `--text-kicker-color`) are settable/re-settable again.
