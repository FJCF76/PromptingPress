...
| section | Text + optional image or content panel; 5 `layout`s (text-only, image-left, image-right, centered, text-panel); band tone and background image through its `_band` role | one of `body` / `body_items` / panel content |
...
| cta | Call-to-action block with `layout` (full-width, inline); band tone and background image through its `_band` role; `title` optional (omit for a standalone button row); optional second button for a primary + secondary pair | `button_text`, `button_url` |
...
Two more components ship with the theme but are **not composable**: `nav` and `footer` are site chrome, rendered on every page by the base template. Putting either into a page composition renders the header or footer twice, so the write is rejected with `template_owned_component`. Set the site logo through the `pp_logo_id` option and the menus through the menu actions; `pp_logo_alt` overrides the logo's alt text on both chrome logos when it should differ from the attachment's own alt metadata (unset, the alt falls back to that metadata and then the site title, so it is never empty). Chrome **styling** is the `pp_site_udc` site option: one `udc` map per chrome component, in the same shape a band takes, reaching every role nav and footer declare. The footer's content (brand blurb, contact block, custom copyright) stays on the `pp_footer_*` options. The footer can also be organised through the same surface: `pp_footer_menu_label` / `pp_footer_contact_label` label its columns, `pp_footer_note` moves the copyright into a delimited bottom bar with a secondary line, and `pp_footer_logo_id` gives it a light logo variant that overrides `pp_logo_id` on the footer only. A second footer menu column (e.g. a distinct Legal column) renders when a menu is assigned to the `footer_secondary` theme location; `pp_footer_secondary_label` is its optional heading, and with no menu assigned the footer is unchanged. A social-icon row is set through `pp_footer_social` — a JSON list of `{network, url}` from a closed set of known networks (x, linkedin, facebook, instagram, youtube, github, tiktok, mastodon) — rendered as accessible inline-SVG icon links under the brand blurb. A chrome background accepts a bounded CSS gradient as well as a plain colour, and a Media Library image via `background.image`, so a dark, gradient or photographic marketing header/footer is expressible without touching the site's global tokens.
...
0 per-instance style slots — the v1 surface has no consumer left. `grid` was the last component declaring one, and its rebuild retired all 38 (#1101). Every component is on the v2 **Universal Design Contract** now, where a band's design is a `udc` map of named ROLES carried in the composition data — and `grid` brought one more thing with it: a single CARD may hold its own map, so one card in an otherwise uniform grid can be dark while its siblings stay light, with the id the engine mints keeping that design attached to the card rather than to position 2.

All ten are on the **Universal Design Contract** (v2) — `testimonials` first, `hero` second, `section` third, `cta` fourth, `faq` fifth, `table` and `embed` sixth and seventh, `stats` and `logos` eighth and ninth, `grid` tenth. Instead of a fixed list of slots someone had to think of in advance, a v2 component declares ROLES — testimonials' quote, card and attribution; hero's title, subtitle, media, surface and its two CTAs; section's nineteen, from the band itself down to a panel row's label and value; cta's eleven, from the band down to each of its two buttons; table's eleven, most of which are surface v1 had no slot for at all — the table fill, the head fill, header and cell ink, the rule widths, the caption; embed's four, which is the whole styling surface a plugin shortcode wrapper has — and every role accepts the core design vocabulary (typography, spacing, border, shadow, background, sizing, motion), plus `layout` on a flex or grid container and `marker` (the colour of a list-marker or separator glyph, #1028) on the four roles that hold one, per breakpoint and in three states (`:hover`, `:focus-visible`, `:active`). Where the vocabulary has no word for what a design needs, a role's `_css` map takes the CSS property directly — raw-CSS parity with a page builder's custom-CSS box, on the same breakpoints, states and token references as everything else, and scoped to the one band rather than loose in a global stylesheet. Shared looks are named `presets` a band applies by name. That is how a brand's serif-italic pull-quote in a white 1px-bordered card became expressible at all.
...
→ **Your band refuses every edit, naming a style slot you never mentioned?** Clear its stored v1 style map first (how-to): [docs/howto-clear-a-stored-v1-style-map.md](docs/howto-clear-a-stored-v1-style-map.md) — one command, and it precedes all eight guides below
→ Migrate a page off the retired v1 style slots (how-to), one per rebuilt component — all eight, the set is complete: [section](docs/howto-migrate-a-section-band-to-v2.md) · [cta](docs/howto-migrate-a-cta-band-to-v2.md) · [faq](docs/howto-migrate-a-faq-band-to-v2.md) · [table](docs/howto-migrate-a-table-band-to-v2.md) · [embed](docs/howto-migrate-an-embed-band-to-v2.md) · [stats](docs/howto-migrate-a-stats-band-to-v2.md) · [logos](docs/howto-migrate-a-logos-band-to-v2.md) · [grid](docs/howto-migrate-a-grid-band-to-v2.md)
...
| **A band that refuses every edit** | [`docs/howto-clear-a-stored-v1-style-map.md`](docs/howto-clear-a-stored-v1-style-map.md) — a page written before its component was rebuilt still stores a v1 `style` map, and since #1101 that map blocks every edit to its band, a title change included. One command clears it, and it comes before every migration guide below |
| **Moving a page off the v1 style slots** | [`docs/howto-migrate-a-section-band-to-v2.md`](docs/howto-migrate-a-section-band-to-v2.md) — the mechanics in full, every retired name and its route. Then the per-component surprises: [`cta`](docs/howto-migrate-a-cta-band-to-v2.md) (its buttons), [`faq`](docs/howto-migrate-a-faq-band-to-v2.md) (its open state), [`table`](docs/howto-migrate-a-table-band-to-v2.md) (its collapsed borders, and the two roles a dark band strands), [`embed`](docs/howto-migrate-an-embed-band-to-v2.md) (how little of arbitrary plugin content a band ink write reaches), [`stats`](docs/howto-migrate-a-stats-band-to-v2.md) (the largest slot map, and a dark band that is four writes), [`logos`](docs/howto-migrate-a-logos-band-to-v2.md) and [`grid`](docs/howto-migrate-a-grid-band-to-v2.md) (the five-write card, and the only component where ONE card can differ from its siblings) |
...
**What exists today:**
- 12 components with schema contracts — ALL of them on the v2 Universal Design Contract, where named roles replace slots (`hero`, `section`, `testimonials`, `cta`, `faq`, `table`, `embed`, `stats`, `logos` and `grid`, plus the two chrome components), and `grid` additionally carries item-grain styling so one card can be addressed on its own (BUILD-SPEC Addendum B)
