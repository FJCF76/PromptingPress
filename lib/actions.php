...
pp_register_action('create_page', [
    'scope'       => 'site',
    'description' => 'Creates a new page with the Composition template. Each composition item is {"component": "name", "props": {...}}. With a composition supplied the call is all-or-nothing: if the composition write is refused — normally because the writer could not take the page lock, error_code composition_lock_failed; the envelope always carries the writer own error_code, so read that rather than assuming which one — the page just created is removed and the call is REFUSED rather than reported as success over an empty page — retry the same call, since normally no page and no slug are left behind; if the message says a page WAS left, it names it (#719).',
    'semantics'   => 'Create. Title is required. Composition defaults to empty array and must be a JSON ARRAY (a list) of components, never an object keyed by position — an object is refused with unexpected_shape and no page is created (#724). Status defaults to "draft". Composition items use the same {"component", "props"} shape as elsewhere. Optional slug sets the canonical route up front (#134) — omit to let WordPress derive one from the title. A page created with no composition is NOT stranded: it can be populated later with update_composition or deleted with trash_page through the operate surface (#358); only component-level edits (add/remove/reorder/update/style_component) require an existing composition first. When a composition IS supplied, the call is all-or-nothing: if the composition write itself is refused, the page just created is deleted again and the call is REFUSED with the writer\'s error_code rather than reported as success over an empty page (#719). Read that error_code before retrying: composition_lock_failed means the lock was busy and an identical retry is right, while composition_not_encodable means the composition you sent cannot be represented as JSON at all and an identical retry will fail the same way (#941).',
    'params'      => [
...
/**
 * The band add_component writes, built ONCE for validate, preview and execute (#1088).
 *
 * Stored exactly as authored (#604): no prop-key rewriting. `style` and `udc` (ruling D2:
 * set semantics) are written only when NON-EMPTY, so an add that carries neither stores a
 * band byte-identical to before. Validation, band-id and token minting are the ordinary
 * ones: the item reaches pp_validate_composition_item() and pp_update_composition() like
 * any other band.
 */
function _pp_build_new_component_item(array $params): array {
...
pp_register_action('add_component', [
    ...
    'validate' => function (array $params) {
        ...
        // The SAME shared engine that validates a composition's items validates the new
        // band — style and udc included — so there is no surface-specific second validator.
        $new_item = _pp_build_new_component_item($params);
    ...
    'preview' => function (array $params): array {
        ...
        // The preview shows the bytes execute will actually persist (#604).
        $new_item  = _pp_build_new_component_item($params);
        $after     = $current;
        if (isset($params['position'])) {
 ...
 *
 * WHY THIS STAYS A ONE-LINE PREDICATE AND NOT A SHAPE CHECK (#841, #842). Two classes have
 * now slipped past it — a prior that decoded to a JSON OBJECT (#841), and a prior that
 * decoded to a LIST whose ELEMENTS are not bands (#842) — and BOTH are answered where the
 * entry FORM is decided, in _pp_normalize_history_ring() (lib/wp.php), not here. Every stage
 * below reaches this resolver through pp_get_composition_history(), so by the time an entry
 * arrives it has already been classified once; re-deciding the form here would give the CLI
 * listing and this refusal two different opinions about the same row, which is exactly how
 * the mis-filed entry came to report `restorable: true` while fataling on selection.
 *
 * SO #842's RULING — "the restore resolver rejects non-replayable entries BEFORE replay" —
 * lands as a DEEPER CLASSIFICATION rather than a second test in this function. The refusal
 * is this one, unchanged and byte-identical in wording; what changed is which rows arrive
 * here already carrying `raw`. Reading the ruling the other way would have satisfied the
 * restore path and left `wp pp operate composition-history` still printing
 * `restorable: true` for the very row this function refuses — the split opinion #841 closed.
 *
...
function _pp_reject_unreplayable_history_entry(array $history, int $idx, array $params) {
    ...
    return new WP_Error(
        ...
        sprintf(
            ...
            //
            // #842 IS THE STRONGEST CASE FOR THAT NEUTRALITY, AND ALSO THE ONE IT
            // STRAINS. Its class decodes fine AND carries no classification at all: the
            // page is HEALTHY, only the slot is unreplayable. So the message must not
            // suggest a decode failure or a corrupt page — which it does not, because
            // "did not decode to a composition" is a statement about what the bytes
            // decoded TO, not about whether decoding worked. That reading is only
            // honest because "a composition" is a defined term here and means a LIST OF
            // COMPONENTS (#841 fixed the container half, #842 the element half); the
            // docs define it the same way on the read side. If that definition ever
            // drifts, this message stops being true for this class first.
            //
            // The wording is deliberately UNCHANGED by #842: it is the string #822
            // routes to the chat's undo card and that a dozen pins assert on, and the
            // class it now also covers does not need a different sentence — it needs
            // the same one to keep meaning what it says.
...
pp_register_action('restore_composition', [
    ...
    'description' => 'Restores a page composition to a prior version recorded in its history ring. Select the target with steps_back (1 = most recent prior state, the default) or history_index (absolute 0-based). history_index takes precedence. A ring slot may instead hold stored bytes that did not decode to a composition — a composition is a JSON ARRAY OF COMPONENTS, so bytes that were unparseable, or valid JSON that is a scalar or a JSON OBJECT, are not one, and neither is a JSON array whose ENTRIES cannot be replayed — an entry that is not an array, or whose "props" is present, not null and not an array (["a","b"], or [{"component":"x","props":"str"}]) — "array" meaning either a JSON object or a JSON list. That last class sits on a page the classifier reports HEALTHY, so it is NOT a corruption signal and must not be treated as one: do not propose a corrupt-page repair write for it, because there is nothing to repair. It is a replayability limit on that one slot. Note the boundary is narrow: an entry that is merely INVALID (no "component" key, say) still restores and still reports its problems in findings (#233). Those slots are preserved so that repairing a corrupt page cannot destroy the only copy of what was there; selecting one is refused with history_entry_not_restorable — read the bytes with `wp pp operate composition-history --post_id=<id>` and select an earlier entry. The selector is also confirmed against the ring at write time: if another writer records a state on the page between your selection and the write, the entry your selector names is no longer the entry you chose, so the restore is refused with history_target_shifted and nothing is written — re-read the ring and select again, preferring a history_index from the fresh listing because steps_back is relative and moves with every concurrent write.',
    'semantics'   => 'Rewrite. The composition is replaced with a prior snapshot captured before an earlier write. Restore is itself a conflict-checked write (records its own history entry), so it can be undone in turn. A ring slot can instead hold stored bytes that did not decode to a composition (a decode_error page, or either sub-case of unexpected_shape — a valid-JSON scalar, or a valid-JSON object), preserved so that repairing a corrupt page cannot destroy the only copy of what was there; selecting that slot is refused with history_entry_not_restorable — read the bytes with `wp pp operate composition-history --post_id=<id>` and select an earlier entry to roll back. Since 1.19.10 that refusal also covers a slot holding a valid JSON LIST whose ENTRIES are not components, which the page classifier calls HEALTHY: replaying one crashed the command outright, so it is now preserved and refused like the others. That is a replayability judgment, not a validity one — every slot carrying a list of components still replays verbatim and still reports, however illegal today rules find its contents (#233). Selection is resolved against the ring the caller read and CONFIRMED against the authoritative ring inside the write lock (#829): a concurrent write that moves what the selector names is refused with history_target_shifted rather than replayed, so whenever that authoritative read succeeds ok:true means the snapshot that was addressed and never whichever snapshot the ring held afterwards. If the authoritative read is not possible or fails, it logs and degrades to the pre-confirmation behavior rather than refusing, so the guarantee is conditional on that read.',
    'params'      => [
        'post_id'          => ['type' => 'int', 'required' => true],
    ...
    'execute' => function (array $params): array {
        ...
        //
        // "VERBATIM" HAS ONE ASTERISK, AND #842 MADE IT LOAD-BEARING ENOUGH TO WRITE DOWN.
        // The claim is about THIS function: it adds, removes, reorders and renames
        // nothing. It is not a claim about pp_update_composition(), which still runs its
        // id-injection loop over whatever this hands it — so an entry that is a LIST
        // (`["a","b"]` as a BAND) or a `props` that is a list comes back reshaped into an
        // object with an injected `id`. Both shapes are deliberately still replayable
        // (they are invalid, not unreplayable, and #233 reserves invalidity for the
        // findings report), so the reshape is the price of not turning this precondition
        // into a validator. Pinned by
        // CompositionHistoryRawPreservationTest::testRestoreStillReplaysInvalidButNonFatalShapes.
        //
    ...
    'validate' => function (array $params) {
        ...
        if ($applied['stranded'] !== []) {
            return new WP_Error('invalid_prop_value', sprintf(
                ...
                . 'but another role or item on the band still references %s. Replace %s in the '
                . 'same call (or send the value yourself under `_tokens`, with a name of your own).',
                ...
                count($applied['stranded']) === 1 ? 'that reference' : 'those references'
            ));
        }
        $test_composition = $composition;
        $test_composition[$params['component_index']] = $applied['item'];
...
// actions that carry a `udc` map (update_component's `udc` param since #1088, and the
// whole-band writers), which is a route they can follow. Made against an
 ...
 * heading role is replaced its value is gone. Pruning leaves that reference dangling; keeping
 * it leaves a mint-shaped name the engine no longer owns, which the gate reads as squatting.
 * Either refusal would name something the caller did not touch, so the name is returned as
 * `stranded` and validate says what actually happened.
 *
 ...
 * @return array{udc: array|null, stranded: string[]} The merged map (null when nothing is
 *         left, and the key is then removed), and any engine-minted token whose own value the
 *         merge removed while something else on the band still references it — validate
...
function _pp_merge_band_udc(array $stored_item, array $patch, array $merged_item): array {
    ...
    // The `props` rule verbatim — the same helper, with no component, so no item-grain
    // carry-forward: a band map has no repeater inside it.
    ...
    if (!array_key_exists('_tokens', $patch)
        ...
        // EVERY place the merged band still names a token, not only the mint's own
        // coordinate: a minted token may legitimately be REUSED by another role or an item
        // (update_composition stores that shape), and pruning it there would leave that
        // reference dangling.
        $referenced = _pp_udc_referenced_token_names($merged);
        foreach ($merged_item_maps as $item_map) {
            $referenced += _pp_udc_referenced_token_names($item_map);
        }
        foreach (array_keys($merged['_tokens']) as $name) {
            ...
            if (!_pp_udc_is_mint_shaped_name($name)
                ...
                continue; // Not the engine's, or its own coordinate still uses it.
            }
            if (isset($referenced[$name])) {
                // Its own value is gone but something else still names it. Neither
                // keeping it (the gate then reads a mint-shaped name the engine no longer
                // owns) nor pruning it (a dangling reference) stores a valid band, so the
                // caller is told which token and why.
                $stranded[] = $name;
                continue;
            }
    ...
    return ['udc' => $merged === [] ? null : $merged, 'stranded' => $stranded];
}
 ...
 * send", and a preview that disagreed with execute is exactly the #604 class. Absent, `null`
 * and an EMPTY `udc`/`style` are all "not sent" (_pp_component_update_carries()).
 *
 * @param array $item    The band as stored.
 * @param array $params  The action params (props / style / udc).
 * @param int   $post_id Pass the page id from EXECUTE ONLY. A non-zero id makes the props
 *                       merge record the item-design carry disclosure
...
function _pp_apply_component_update(array $item, array $params, int $post_id = 0): array {
    ...
    $style_before = isset($item['style']) && is_array($item['style']) ? $item['style'] : [];
    ...
    if (_pp_component_update_carries($params, 'style')) {
        $style_after = _pp_merge_component_props($style_before, $params['style']);
        if (empty($style_after)) {
            unset($item['style']);
        } else {
            $item['style'] = $style_after;
        }
    }
    ...
    if (_pp_component_update_carries($params, 'udc')) {
        ...
        $merge = _pp_merge_band_udc($stored_item, $params['udc'], $item);
        $stranded = $merge['stranded'];
        if ($merge['udc'] === null) {
            unset($item['udc']);
        } else {
            $item['udc'] = $merge['udc'];
...
/**
 * Did the caller send this update_component payload (#1088)? Absent and `null` are not
 ...
 * except `_tokens`, which is diffed PER TOKEN: a prune touches a few names, and a row
 * carrying the whole token map on both sides grew the envelope with the band's total token
 * count rather than with the change (measured: ~1 MB for a one-role edit at 12,000 tokens).
 */
function _pp_diff_udc(array $before, array $after, int $index): array {
    ...
    foreach (array_unique(array_merge(array_keys($before), array_keys($after))) as $key) {
        ...
        if ($key === '_tokens' && (is_array($from) || $from === null) && (is_array($to) || $to === null)) {
            $from = $from ?? [];
            $to   = $to ?? [];
            foreach (array_unique(array_merge(array_keys($from), array_keys($to))) as $name) {
                if (($from[$name] ?? null) !== ($to[$name] ?? null)) {
                    $changes[] = [
                        'path' => 'composition[' . $index . '].udc._tokens.' . $name,
                        'from' => $from[$name] ?? null,
                        'to'   => $to[$name] ?? null,
                    ];
                }
