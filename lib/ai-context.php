...
function pp_ai_system_prompt(): string {
    ...
    // GRAMMAR (#1087). Two of these disclosures used to ride inside the `length-or-none`
    ...
    $parts[] = 'SLOT NAMES YOU MAY MEET ON AN AGED PAGE, ALL RETIRED, none of them writable '
        . 'any more: `--stats-max-width` and `--stats-bg-position` went with stats\' rebuild, '
        . '`--faq-body-measure` was the last text measure and left with faq\'s, and '
        . '`--logos-image-size` is gone too — every one of those components declares ZERO '
        . 'style slots now, so `style_component` refuses them with `no_style_slots`. The '
        . 'replacement is a `udc` write on the band: a width cap is the role\'s '
        . '`sizing.max-width`, an image cap is `sizing.max-height`, and a band background\'s '
        . 'focal point is `_band` -> `background.position`. If you are editing a page built '
        . 'before the rebuild and you find one of these names in a stored `style` map, clear '
        . 'the map and write the equivalent `udc` values instead.';
 ...
 *
 * THE BLOCK IS SELF-DELETING, which is the real point. `grid` is the last component on
 * style slots; when its rebuild lands, this returns '' and its caller drops the whole
 * section. Nobody has to remember to delete it, which is the only kind of cleanup that
 * reliably happens.
 *
