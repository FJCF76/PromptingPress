...
function pp_ai_system_prompt(): string {
    ...
    $parts[] = 'A PROP A COMPONENT USED TO HAVE, AND NO LONGER DOES, IS REFUSED WITH ITS OWN CODE `retired_prop`, not `unknown_prop`, and the refusal names the v2 surface that replaced it — the hero and testimonials styling props all moved into the band\'s `udc` map. You will meet these on pages built before the rebuild. TO CLEAR ONE, SEND IT AS null: `update_component` with `{"<prop>": null}` removes the stored key, and that is the only way to remove a key the schema no longer declares. ONE BAND AT A TIME IS ENOUGH — `update_component` validates the band it targets, so a stale prop on one band does not block edits to another, and a page with stale props on several bands is cleared one band per call. The page\'s other problems are still reported on the accepted envelope\'s `findings` at severity `error`, so read them rather than assuming the page is clean. The EXCEPTION is a duplicate `props.id` across bands: that is a property of the whole page, so it refuses an edit to ANY band until you repair the ids through `update_composition`, and the refusal says so.';
