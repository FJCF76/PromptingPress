...
function pp_ai_system_prompt(): string {
    ...
    $parts[] = 'LAYOUT: the `layout` group sets how a CONTAINER arranges its children, and it is permitted only on roles whose box IS one (the catalog lists each role\'s groups — a heading or a paragraph does not have it, because `justify-content` on a text block paints nothing). Its parameters: `columns` (grid tracks), `orientation` (`row`, `row-reverse`, `column`, `column-reverse`), `wrap` (`nowrap`, `wrap`, `wrap-reverse`), `justify` (main-axis packing: `center`, `start`, `end`, `flex-start`, `flex-end`, `left`, `right`, `space-between`, `space-around`, `space-evenly`, `stretch`, `normal`) and `align` (cross-axis: `center`, `start`, `end`, `flex-start`, `flex-end`, `stretch`, `baseline`, `first baseline`, `last baseline`, `normal`). Any positional keyword may carry `safe` or `unsafe` (`"safe center"`), which stops a centred item overflowing its container unreachably. HOW ONE BOX PLACES ITSELF is NOT here — it is `sizing.align-self`, on the child role itself, taking the `align` keywords plus `auto` and `self-start`/`self-end`. So "centre this panel beside the text" is `{"panel": {"sizing": {"align-self": "center"}}}`, while "centre both columns" is `{"columns": {"layout": {"align": "center"}}}`.';
    $parts[] = 'COLUMNS, AND THE ONE THING IT DOES BESIDES SET TRACKS: `layout.columns` takes a whole number from 1 to 12 — `{"columns": {"layout": {"columns": 4}}}` gives four equal columns — or an explicit track list (`"3fr 2fr"`, `"240px 1fr"`, `"repeat(auto-fit, minmax(20rem, 1fr))"`, `"minmax(0, 1fr) minmax(0, 1fr)"`; at most 12 tracks, one `repeat()` per list, no nesting, and no `calc()` inside a track in this cut). A count is expanded to `repeat(N, minmax(0, 1fr))` so a long unbroken word can never widen a track and scroll the page sideways. SETTING IT MAKES THE BOX A GRID: the engine emits `display: grid` beside your value, because otherwise the value would paint nothing wherever the theme lays that box out with flexbox — and a value that stores and paints nothing is exactly what this system refuses to ship. The consequence to plan for: on that box, at the breakpoints you set it, `orientation` and `wrap` stop applying, because they are flexbox parameters. THE PHONE IS NOT STACKED FOR YOU: `d` is the base and applies at EVERY width, so `{"columns": 4}` is four columns on a phone too. Write `{"columns": {"d": 4, "p": 1}}` when you want the ordinary stack, exactly as with any other responsive value.';
