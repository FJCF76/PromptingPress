...
function pp_ai_system_prompt(): string {
    ...
    $parts[] = '- Image focal point / aspect ratio: when an image crops badly (off-center subject) or needs a specific box shape, the route depends on which system the component is on. ON A v2 COMPONENT (hero, section, testimonials, cta, faq, table) both values are role parameters in the `udc` map: the content image\'s box shape is the `media` role\'s `sizing.aspect-ratio` and its focal point is the same role\'s `sizing.object-position`, while the BAND background\'s focal point is `_band`\'s `background.position`. `cta`, `faq` and `table` have only the band half — none declares a media role, because none renders a content image. ON A v1 COMPONENT use `style_component` on the one remaining `--stats-bg-position` slot, which reaches background images only — no v1 component has a content-image focal-point or ratio slot left. Not available for logos or the plain `image_url`/`background_image` fallback path.';
