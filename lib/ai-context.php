...
function pp_ai_system_prompt(): string {
    ...
    $parts[] = 'A component listed above with "UDC roles" is on the v2 styling system and has NO style slots: `style_component` will refuse it with `no_style_slots`. Style it with a `udc` map on the BAND, alongside `props`. To restyle ONE existing band, send `update_component` with a `udc` param (#1088): it is merged into the band\'s stored map BY ROLE — a role you send replaces that role\'s map whole (so send every value the role should keep), `null` removes a role, and roles you do not send are kept; `props` is then optional. `add_component` takes `udc` for the band it adds, and `update_composition` / `create_page` carry it inside whole bands. Pass `expected_version` so an edit that landed since you read is refused rather than overwritten.';
    // THE ONE EXCEPTION, said immediately after the rule it qualifies (#1101). Without
    // it the paragraph above reads as a flat denial that `update_component` carries any
    // design at all, which is false at item grain and is the grain the owner's own
    // production design is written in.
    $parts[] = 'ONE EXCEPTION, and it is the per-CARD design: a component whose catalog entry lists "ITEM-GRAIN roles" lets a SINGLE entry of its repeater prop carry its own `udc` map, written as an ordinary field of that entry (`{"items": [{"title": "...", "udc": {"card": {"background": {"fill": "#14141F"}}}}]}`). Because it rides inside `props`, `update_component` DOES reach it — that is the one design surface a patch can write. Send the whole repeater array when you patch it, and send every entry you want to keep: the array is replaced, not merged. The entry\'s `id` is minted by the engine and carried forward for you; never write one yourself. USE IT FOR "this ONE card is different" and nothing else — a value that should apply to every entry belongs on the band map, where one write reaches them all.';
