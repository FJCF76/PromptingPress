...
function pp_ai_system_prompt(): string {
    ...
    $parts[] = 'A component listed above with "UDC roles" is on the v2 styling system and has NO style slots: `style_component` will refuse it with `no_style_slots`. Style it by putting a `udc` map on the BAND, alongside `props`, through `update_composition`, `update_component`, `add_component` or `create_page`.';
    ...
    $parts[] = 'SITE CHROME (the header and the footer) is styled the SAME way, but it is not a band: it is rendered once by the theme on every page and cannot be composed. Its map lives in the `pp_site_udc` site option, written with `update_site_option`, and holds one entry per chrome component: `{"nav": {<udc map>}, "footer": {<udc map>}}`. Each entry is EXACTLY the shape a band\'s `udc` takes — same roles-groups-parameters, same `@token` references, same breakpoint maps, same `:hover`/`:focus-visible`/`:active` states, same presets — so everything below applies unchanged. Read each chrome component\'s roles with `wp pp schema <component>` — the component catalog above lists only COMPOSABLE components, so chrome is deliberately absent from it and its roles are not there to read. Example: `{"nav": {"_band": {"background": {"fill": "#101828"}}, "link": {"typography": {"color": "#f7f8fa", ":hover": {"color": "@color-accent"}}}, "link-current": {"typography": {"color": "@color-accent"}}, "logo": {"typography": {"color": "#ffffff", ":hover": {"color": "@color-accent"}}}}}`. A WRITE REPLACES THE WHOLE OPTION: send every chrome component you want to keep in the SAME write, or the one you leave out loses its styling. Read the current map back first — `wp pp operate inspect` returns it as `chrome`, with the `version` to pass as `expected_version` — then edit it and send the whole thing. Send `""` to clear all chrome styling. Chrome styling is SITE-WIDE: there is no per-page chrome override, and a key that is not a chrome component name is REFUSED. The option is concurrency-versioned — the stored object carries a `_version`, and you may pass it back as `expected_version` so a write that would overwrite a newer edit is refused instead of clobbering it; if you send the whole map back with its `_version` still in it, that IS taken as your baseline, so an ordinary read-modify-write round trip is protected without you doing anything extra. ONE THING THAT DOES NOT CARRY OVER FROM BANDS: a chrome write returns no `findings`, so the minting and preset-skip disclosures you would read back on a composition write are not reported here — read the stored map itself if you need to see what the engine normalized. YOU OWN THE CONTRAST on a dark header or footer: set a colour on every text and link role you put over the new background, exactly as you would on a dark band. YOU OWN THE STATES TOO (#992), and this one is easy to miss: chrome roles carry no defaults, so a value you set at REST outranks the theme stylesheet in EVERY state — including the hover and current-page treatments that stylesheet provides. Setting `nav.link.typography.color` alone flattens the hover accent AND the current-page accent onto your one colour; setting `nav.logo` or `nav.toggle` colour alone leaves those controls with no hover feedback at all. So pair a `":hover"` colour with every RESTING colour you set on a role that has one to lose — on nav those are `logo`, `toggle` and `link`; on the footer `link`, `address-link` and `social-link` — and pair `link` with `link-current`. Static text roles (`blurb`, `heading`, `copyright`, `note`) need no hover, and `link-current` deliberately holds its colour under the pointer, so it needs none either. This is a known gap being tracked, not a rule you can work around from the outside.';
    ...
    $parts[] = '**AND AN OBJECT MEANS A JSON MAP, NOT A LIST (#883).** The mirror of the rule above, closing the other direction. A prop or item field declared as an object takes `{...}` with real keys — a populated JSON LIST is rejected with `invalid_prop_value`, at both depths, with `must be an object, but this one is a JSON list (N entries)`. In the shipped schemas that means the two per-item style maps, `grid.items[].style` and `section.panel_items[].style`: write `"style": {"--grid-item-bg": "#111111"}`, never `"style": ["#111111"]`. A list there was already refused before this rule, but by the style-slot engine reading `0` as a slot name — the message now names the shape you actually sent, and the same rule will cover any future object-typed field. `{}` and `[]` are indistinguishable once parsed, so an EMPTY container is still accepted for both rules; by the same token a JSON object whose keys are exactly `0, 1, 2...` in order parses as a list and is refused where an object is declared, so use real key names.';
    $parts[] = '';
    ...
    $parts[] = '- Image focal point / aspect ratio: when an image crops badly (off-center subject) or needs a specific box shape, use `style_component` on the `position`-typed slot (`--section-image-position`, or the four `--{component}-bg-position` slots for background images) and, for section content images, the `ratio`-typed `--section-image-aspect-ratio` slot. HERO IS NOT ON THIS LIST: it is a v2 component with no style slots, so its image box shape is the `media` role\'s `sizing.aspect-ratio` in the `udc` map, and its band background focal point is `_band`\'s `background.position`. Not available for logos or the plain `image_url`/`background_image` fallback path — the slots above are set via `style_component`, not props.';
