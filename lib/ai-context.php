 ...
 * measurable.
 *
 * RAISED 92000 → 92200 (#1141/#1142, PR-2 of #1145), for two contract facts an author meets on
 * the write: a raw `_css` `background` now cancels every background value at its coordinate
...
function pp_ai_system_prompt(): string {
    ...
    $parts[] = 'MOTION: the `motion` group carries `transition-duration` (a time, e.g. `"150ms"` or `"0.2s"`) and `timing-function` (a keyword — `linear`, `ease`, `ease-in`, `ease-out`, `ease-in-out`, `step-start`, `step-end` — or `cubic-bezier()` with four numbers whose 1st and 3rd are between 0 and 1 while the 2nd and 4th may be any number including negative, or `steps()` with a positive integer of at most 1000 and an optional `jump-start`/`jump-end`/`jump-none`/`jump-both`/`start`/`end`, where `jump-none` additionally needs two or more steps). MOTION VALUES ARE CASE-SENSITIVE: write `ease`, not `EASE`. Both params default to the theme\'s own values, `150ms` and `ease`. You do NOT need to handle `prefers-reduced-motion`: the engine emits that guard for every motion value, and there is no parameter for it.';
    // THE MARKER CAVEAT (#1028). The catalog offers `marker` on `body` and `panel-list` by name
    // only, and a list left on the default `disc` is a native ::marker that ignores the variable:
    // without this line the in-admin model writes an accepted value that paints nothing.
    $parts[] = 'MARKER: `marker.color` paints only a drawn glyph (separator, `check`/`dash`/`arrow`); a `disc` list ignores it.';
    ...
    $parts[] = 'RAW CSS (`"_css"`) — THE ESCAPE HATCH, AND WHEN NOT TO USE IT. Beside a role\'s '
        ...
        . 'SET BOTH, `_css` WINS — a raw `color` outranks `typography.color` on the same role, a raw `background` cancels every `background` value there (image and scrim included), and '
