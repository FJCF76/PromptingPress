...
function pp_ai_system_prompt(): string {
    ...
    $parts[] = '- Image focal point / aspect ratio: when an image crops badly (off-center subject) or needs a specific box shape, the route depends on which system the component is on. ON A v2 COMPONENT (hero, section, testimonials, cta, faq, table, embed, stats, logos) both values are role parameters in the `udc` map: the content image\'s box shape is the `media` role\'s `sizing.aspect-ratio` and its focal point is the same role\'s `sizing.object-position`, while the BAND background\'s focal point is `_band`\'s `background.position`. `cta`, `faq`, `table`, `embed` and `stats` have only the band half — none declares a media role, because none renders a content image through one. `logos` is the odd member: its images are ITEMS rather than a band media slot, so its caps are the `image` and `image-labeled` roles\' `sizing.max-height` and it has no focal point at all. ON A v1 COMPONENT there is no such route left at all: grid is the only component still on style slots and it declares no background-image, focal-point or ratio slot — `--stats-bg-position` was the last one and retired with stats at #1066. Not available for the plain `image_url`/`background_image` fallback path.';
