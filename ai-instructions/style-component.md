...
> **This step is for v1 components only.** `hero`, `section` and `testimonials` are on the
> Universal Design Contract: they declare no style slots and no recipes, and a
> `style_component` call naming one is refused. Style them through the band's `udc` map —
> see the Universal Design Contract section below. The examples here use `cta`, which is
...
wp pp action execute style_component --run-id=<uuid> --params='{
  "post_id": 19,
  "component_id": "pp-a1b2c3d4",
  "recipe": "accent-framed",
  "style": {
    "--cta-heading-size": "clamp(3rem, 6vw, 5rem)"
  }
}'
```
...
such as `--cta-heading-measure`) still rejects it, and there `100%` remains the way to
