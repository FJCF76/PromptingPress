...
**A styling revision goes through `update_composition`, and that is not a violation of the rule above — it is the only route the engine offers.** `update_component` declares exactly four parameters (`post_id`, `component_index`, `component_id`, `props`): it carries no `udc` and no `style`, so it cannot change how a band looks. `add_component` carries `style` but no `udc` either, and `style_component` is refused with `no_style_slots` on every component but `grid`. So a `udc` edit is necessarily a read-modify-write of the WHOLE composition:
