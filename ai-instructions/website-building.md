...
| Dark or gradient marketing header | WordPress options | `update_site_option` action with key `pp_site_udc`, the `nav` entry: `_band.background.fill` for the bar, `logo` / `toggle` / `link` / `link-current` for the ink, `menu` / `submenu` for the two panels. The header is template-owned, so this is its ONLY styling surface — never invert the site's global tokens to fake a dark header |
