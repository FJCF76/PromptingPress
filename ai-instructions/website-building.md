...
| Dark or gradient marketing header | WordPress options | `update_site_option` action with key `pp_site_udc`, the `nav` entry: `_band.background.fill` for the bar, `container` for the row's height and gap, `logo` / `toggle` / `link` / `link-current` for the ink, `menu` / `submenu` for the two panels. Pair every colour you set at rest with its `":hover"`, and pair `link` with `link-current` — a chrome value set at rest outranks the stylesheet in every state, so styling only the resting colour silently drops the hover accent and the you-are-here marker (#992). The header is template-owned, so this is its ONLY styling surface — never invert the site's global tokens to fake a dark header |
