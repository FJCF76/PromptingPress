...
| **A band's ordinary appearance** — the type, colour, spacing, border, radius, shadow, size or motion of anything inside one band | `_pp_composition` post meta (the band's own `udc` map) | The band's `udc` map: `role -> group -> parameter -> value`, written to ONE band by `update_component`'s `udc` param (merged BY ROLE: a sent role replaces that role's map, `null` removes it), set on a new band by `add_component`'s `udc`, or carried inside whole bands by `update_composition` / `create_page`. The nine groups are `typography`, `spacing`, `border`, `background`, `sizing`, `shadow`, `layout`, `motion` and `marker` (the colour of a list-marker or separator glyph, #1028). Read a component's roles with `wp pp schema <component>` before writing one. This is the ordinary case and since #1101 it covers all TEN composable components; see `ai-instructions/style-component.md` |
| A per-instance style slot | RETIRED — nothing declares one since #1101 | `style_component` is refused on **every** component with `no_style_slots`. Band styling is the `udc` map row above; ONE card of a repeater is that entry's own `udc` map (Addendum B, `grid` today) |
| Band tone (a light tinted band, a dark band) | the band's `udc` map | **NO component carries a `theme` prop** — `grid` was the last and it went at #1101 (`testimonials` #958, `hero` #986, `section` #1023, `cta` #1026, `faq` #1046, `table`/`embed`/`stats`/`logos` #1066). A tone is a bundle of designable values, so say it directly: `_band` `background.fill` plus the text roles' `typography.color`. Sending `theme` is refused with `retired_prop`, and the refusal names the route |
...
- Prefer a `layout` change, an image, or a different component (grid, stats) to break up rhythm — not a narrower/tighter version of the same layout. A tone written into the band's `udc` map is the other cheap lever, on every band including `grid`.
