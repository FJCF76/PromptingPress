...
| Component structure (steps layout, image side, split ratio) | `_pp_composition` post meta | `update_component` action (set the `layout` prop) |
| **A band's ordinary appearance** — the type, colour, spacing, border, radius, shadow, size or motion of anything inside one band | `_pp_composition` post meta (the band's own `udc` map) | The band's `udc` map: `role -> group -> parameter -> value`, carried by `update_composition` or `create_page` ONLY (`update_component` and `add_component` declare no `udc` parameter, so a styling edit is a read-modify-write of the whole composition). The eight groups are `typography`, `spacing`, `border`, `background`, `sizing`, `shadow`, `layout` and `motion`. Read a component's roles with `wp pp schema <component>` before writing one. This is the ordinary case and it covers all NINE v2 components; see `ai-instructions/style-component.md` |
| A per-instance style slot | `_pp_composition` post meta (the component's `style` key) | `style_component` action — **`grid` only.** It is the one component with style slots; on any of the nine v2 components the call is refused with `no_style_slots`. Read grid's slots with `wp pp schema grid` rather than from a count written down here, and the styling belongs in the `udc` map row above |
| Band tone (a light tinted band, a dark band) | depends on the component's contract | **`grid` is the ONLY component left carrying the `theme` prop** — `update_component` with `muted` (light tinted band) or `inverted` (dark band). **Every other band component** (`testimonials` #958, `hero` #986, `section` #1023, `cta` #1026, `faq` #1046, `table` and `embed` #1066, `stats` and `logos` #1066) has NO `theme` prop: a tone is a bundle of designable values, so say it directly in the band's `udc` map — `_band` `background.fill` plus the text roles' `typography.color`. Sending `theme` to one of those is refused with `retired_prop`, and the refusal names the route |
| A CSS property the design needs and PromptingPress has NO parameter for (`opacity`, `mix-blend-mode`, `-webkit-line-clamp`, `filter`, …) | `_pp_composition` post meta (the band's own `udc` map) | The role's `"_css"` map (#1079) — a plain property => value list beside that role's groups, at role or `_band` grain, e.g. `"quote": {"_css": {"opacity": "0.75"}}`. **Structured params FIRST:** if a group owns the property, use the parameter — `_css` buys nothing there and costs you the catalog and the type check, and a property the vocabulary knows KEEPS its parameter's grammar inside `_css` anyway (`color` still refuses `red`). Same breakpoint maps, `@token` references and `:hover`/`:focus-visible`/`:active` states as any other value. This is NOT WordPress Additional CSS: it is scoped to the one band, stored in the composition, and covered by the same validation, versioning and undo as everything else you write. See `ai-instructions/style-component.md` |
...
| Dark marketing footer + blurb/contact/copyright | WordPress options | `update_site_option` action. STYLING: `pp_site_udc`, the `footer` entry — a `udc` map in the same shape a band takes, reaching every role the footer declares. TEXT: `pp_footer_blurb` / `pp_footer_contact` / `pp_footer_copyright` (empty `pp_footer_copyright` keeps the default line) |
...
| Footer social-icon row | WordPress option (JSON) | `update_site_option` action (key `pp_footer_social`, a JSON string: an ordered list of `{network, url}`, e.g. `[{"network":"x","url":"https://x.com/acme"},{"network":"linkedin","url":"https://linkedin.com/company/acme"}]`). `network` must be one of the closed set `x`, `linkedin`, `facebook`, `instagram`, `youtube`, `github`, `tiktok`, `mastodon`; `url` must be an http(s) profile URL. Renders accessible inline-SVG icon links under the brand blurb (colour is the `social-link` role). Unknown networks / non-URL values are rejected; `""` clears the row. |
| Dark or gradient marketing header | WordPress options | `update_site_option` action with key `pp_site_udc`, the `nav` entry: `_band.background.fill` for the bar, `container` for the row's height and gap, `logo` / `toggle` / `link` / `link-current` / `submenu-toggle` for the ink, `menu` / `menu-list` / `submenu` for the panels and the list. Since #994 every role carries the header's resting appearance as a default, so styling only the resting colour KEEPS the hover accent and the you-are-here marker (#992 is fixed) — but set `submenu-toggle` whenever you set `link`, because the dropdown chevron is the link's SIBLING and nothing can make it follow the link colour on its own (#995). The header is template-owned, so this is its ONLY styling surface — never invert the site's global tokens to fake a dark header |
| Bringing an image onto the site as a locally-owned asset | Media Library (new attachment, or reused if that source URL was already imported) | `import_media` apply — give it EITHER a remote `url` OR a server-local absolute `file` (a brand-kit asset such as a logo, favicon, or OG card that lives on the operator machine, not a public URL; #490), exactly one. Returns `{attachment_id, url, action}` (`action` is `import`, or `reused` when re-importing the same `url`; the `file` source is copied so your original is untouched). Pass `url` to `image_url`/`logo_url` (`background_image` is retired on every component — a BAND background is `_band` -> `background.image` in the `udc` map and takes the `attachment_id`, never a url); on hero/section and on logos/grid/testimonials items also pass `attachment_id` as `image_id` for responsive srcset output; pass `attachment_id` to attachment-ID-only options (`pp_logo_id`, `site_icon`, `pp_og_image`). |
...
| Redirect an old/renamed URL to its new location | `pp_redirects` option | `create_redirect` action (`from` path → same-site `to` target, 301 default / 302 optional). Use right after `update_page_slug` so the old URL 301s instead of 404ing. `remove_redirect` / `list_redirects` to manage them. `to` must be same-site (path or absolute same-host URL). **Create-or-REPLACE, and the result tells you which happened (#887):** a second create for the same `from` silently overwrites the row already there, and the step returns `changes[0].from` as the row it replaced (`{to, code}`) or `null` when it genuinely created one. Read it before assuming you added a redirect rather than destroying one — `list_redirects` first if you need to know beforehand |
| Custom/webfont loading and typography ("use Poppins for headings") | `pp_font_urls` option + `pp_token_overrides` option | `enqueue_font` apply with `url` + `family` + `apply_to` (`heading`\|`body`\|`both`) — one call both loads the stylesheet and points `--font-heading`/`--font-body` at it. `enqueue_font` with only `url` loads the font but changes nothing visible; it is not a substitute for `apply_to`. When `family` is omitted the family name is read out of the URL's `family=` parameter and must satisfy the font-family grammar; a name carrying punctuation or non-ASCII outside quotes is refused, and the fix is to pass `family` explicitly, quoted. A heading face set this way does not reach the `stats` display numbers — those follow the body font (deliberately: the `number` role declares no `family` default, so the inherited face keeps landing) until the band's `udc` map sets `number` -> `typography.family` and, if the display face wants it, `typography.weight` |
...
Squeezing a band is not a fix for a page that feels cramped, memo-like, or visually weak (issue 51). Reaching for the same narrowing or tightening repeatedly across a page is a symptom, not a solution — it produces a composition that is structurally valid but visually worse. If a page feels wrong:
- Prefer a `layout` change, an image, or a different component (grid, stats) to break up rhythm — not a narrower/tighter version of the same layout. On a `grid` band a `theme` change is the other cheap lever; on every other band the equivalent is a tone written into the band's `udc` map, not a prop.
...
- Three or more consecutive components with `width: narrow`, or three or more with `spacing: compact`, will surface as a `consecutive_narrow_width`/`consecutive_compact_spacing` composition smell in `wp pp check page` — treat that as a signal to change the component or its content, not to suppress the warning by varying the count. **NO COMPONENT DECLARES `width` OR `spacing` ANY MORE** — `hero` was the last and shed both at #986 — so these two smells fire only on legacy stored data, and the fix on a page that trips one is to clear the stale prop (`update_component` with `{"width": null}`), not to re-pick a value.
