...
| A per-instance style slot | `_pp_composition` post meta (the component's `style` key) | `style_component` action — **`grid` only.** It is the one component with style slots (38); on any of the nine v2 components the call is refused with `no_style_slots`, and the styling belongs in the `udc` map row above |
