...
| Band tone (a light tinted band, a dark band) | depends on the component's contract | **v1 components** (`embed`, `grid`, `logos`, `stats`) still carry the `theme` prop — `update_component` with `muted` (light tinted band) or `inverted` (dark band). **v2 components** (`testimonials` #958, `hero` #986, `section` #1023, `cta` #1026) have NO `theme` prop: a tone is a bundle of designable values, so say it directly in the band's `udc` map — `_band` `background.fill` plus the text roles' `typography.color`. Sending `theme` to one of those is refused with `retired_prop`, and the refusal names the route |
