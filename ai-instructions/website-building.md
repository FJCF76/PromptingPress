...
| Dark marketing footer + blurb/contact/copyright | WordPress options | `update_site_option` action (colors: `pp_footer_bg` / `pp_footer_text` / `pp_footer_link_color`, any CSS color the style-slot validator accepts; text: `pp_footer_blurb` / `pp_footer_contact` / `pp_footer_copyright`, empty `pp_footer_copyright` keeps the default line) |
