...
| **A band's ordinary appearance** — the type, colour, spacing, border, radius, shadow, size or motion of anything inside one band | `_pp_composition` post meta (the band's own `udc` map) | The band's `udc` map: `role -> group -> parameter -> value`, written to ONE band by `update_component`'s `udc` param (merged BY ROLE: a sent role replaces that role's map, `null` removes it), set on a new band by `add_component`'s `udc`, or carried inside whole bands by `update_composition` / `create_page`. The eight groups are `typography`, `spacing`, `border`, `background`, `sizing`, `shadow`, `layout` and `motion`. Read a component's roles with `wp pp schema <component>` before writing one. This is the ordinary case and since #1101 it covers all TEN composable components; see `ai-instructions/style-component.md` |
