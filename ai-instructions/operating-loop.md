...
**`ok: true` is not the whole result — read `findings` (#687).** Every accepted composition write returns a `findings` list describing the composition it just stored: `severity: warning` advisories (notably `inert_slot`, "the value is stored and reported as applied, but nothing on the page reads it") and `severity: error` findings on bands current rules reject. A write can succeed and paint nothing, so treat a non-empty `findings` on your own edit as part of the edit's result, not as background noise: fix the band it names in this same EDIT step, or record why you are leaving it. On a single write an empty list is the confirmation that the write did what you asked. **In a BATCH it is not, if the batch rolled back:** each step's report was built when that step succeeded, before a later step's failure reverted everything, so a rolled-back batch returns reports — empty or not — describing compositions that no longer exist. After `rolled_back: true`, re-read the page; do not read any step's `findings` as the state of anything.
