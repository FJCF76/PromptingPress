...
| `invalid_prop_value` (ERROR) | An out-of-set enum value, an out-of-range number, or a value whose shape does not match the prop's declared `type` — including a non-string scalar (`42`, `3.14`, `true`, `false`) on a text prop, at both depths since #707 | For an enum or a bounded number, use one of the advertised values; the message names the accepted set. For a type mismatch, the message names the prop (and, one level down, the item and field) and the shape it wanted: **quote the value** — `"number": "99%"`, `"image_url": "/wp-content/uploads/logo.png"` — or drop the key to leave it unset. **Repair every offending band in ONE `update_composition` write**: `check page` lists them all, and because every mutating action validates the WHOLE composition, `update_component` (and `wp pp operate patch`, which routes through it) cannot land while a SECOND band is still bad — a single-band repair is refused by the band it is not touching. `update_component` works once exactly one band is left to fix |
