...
| `invalid_style_value` / `invalid_style_slot` (ERROR) | A style slot the component does not declare, an unusable value, or a non-scalar value where a scalar belongs (#622). **`grid` is the only component with style slots**, so on any of the nine v2 components a `style` map is refused outright with `no_style_slots` | Set a declared slot with a scalar value, or drop it. On a v2 component the styling belongs in the band's `udc` map instead |
| `unknown_udc_role` (ERROR) | The band's `udc` map names a role the component does not declare — a typo, or a role borrowed from a different component. The same code also answers a `udc` map on `grid`, which is on no design contract at all | Use a role the component declares; the message lists them. Read them with `wp pp schema <component>` |
...
| `invalid_udc_value` (ERROR) | A parameter value the group's grammar refuses, including a `@token` reference that is not a registered design token — `@pp-band-padding` and `@pp-band-heading-size` are the ones that catch people, because they are legal as shipped role DEFAULTS and illegal in an authored map | Use a literal or a registered token. `update_design_token` lists what is registered |
