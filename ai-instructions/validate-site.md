...
| `invalid_style_value` / `invalid_style_slot` (ERROR) | A style slot the component does not declare, an unusable value, or a non-scalar value where a scalar belongs (#622). **No component declares style slots since #1101**, so a `style` map is refused outright with `no_style_slots` on every one of them | Drop the `style` map. The styling belongs in the band's `udc` map — or, for ONE card of a repeater, in that entry's own `udc` (Addendum B) |
